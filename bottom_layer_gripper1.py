...
def Feasible_Grasp(
    ...
    if (
        MultiPoint([F1_position, F2_position]).distance(LineString(profile_environment))
        < 0.1
    ):
        return False
    if (
        ...
        .buffer(-accuracy)
        .intersects(MultiPoint([F1_position, F2_position]))
    ):
        return False
    if (
...
def whether_finger_contact_pair_feasible(ini_vertex_position, F1_index, F2_index):
    ...
    if Point(F1_position).distance(Point(F2_position)) > 0.82:
        return False
    if (
        MultiPoint([F1_position, F2_position]).distance(MultiPoint(ini_vertex_position))
        < 0.1
    ):
        return False
    return True
...
def feasible_finger_contact_pair_set(ini_vertex_position):
    ...
    for grasp_index in temp_list:
        if (
            whether_finger_contact_pair_feasible(
                ini_vertex_position, grasp_index[0], grasp_index[1]
            )
            == False
        ):
...
def feasible_fcp_avoid_env_collision(
    current_vertex_position, feasible_fcp_set, profile_environment, accuracy=0.002
):
    avoid_index_set = []
    ...
    for fcp in feasible_fcp_set2:
        gripper_pose_set = from_index_to_gripper_pose_set(
            current_vertex_position, fcp[0], fcp[1]
        )
        if gripper_pose_set == []:
        ...
        if xtuple(object_pose_discrete) in fcp_set_index_memory:
            ...
            feasible_fcp_set1, feasible_gripper_pose_set1 = (
                feasible_fcp_avoid_env_collision(
                    object_pose_trajectory[i], fcp_index_list_temp, profile_environment
                )
            )
...
def SuitableGrasp_tip_instantaneous(
    ...
    for i in range(len(contact_index)):
        if re.match("E", contact_index[i]) is not None:
    ...
    for i in range(len(object_pose_trajectory_plus_ini)):
        ...
        if (
            ...
            for j in range(len(moveobj_link_end_equal_i_gripper_pose)):
                ...
                current_F1, current_F2, _ = gripper_profile(
                ...
                (
                    ...
                    object_pose_trajectory_plus_ini[i], current_F1, current_F2
                )  # need change for gripper 2
                    ...
                    if action == "MoveGripper":
                        ...
                        for item in target_gripper_pose_set:
                            for feasible_gripper_pose in feasible_gripper_pose_set_list[
                                ...
                                if (
                                    abs(item[0][0] - feasible_gripper_pose[0]) < 0.005
                ...
                for action in feasible_action_list:
                    ...
                    for item in target_gripper_pose_set:
                        for (
                            ...
                            if (
                                abs(item[0][0] - feasible_gripper_pose[0]) < 0.005
                                and abs(item[0][1] - feasible_gripper_pose[1]) < 0.005
...
def A_star_find_action_path(
    ...
    if not whether_AdjustGraspAction_feasible(
        ...
        for discrete_gripper_pose in ini_feasible_gripper_pose_subset:
            if (
                abs(ini_gripper_pose[0] - discrete_gripper_pose[0]) < 0.01
                and abs(ini_gripper_pose[1] - discrete_gripper_pose[1]) < 0.01
...
def AdjustGraspAction_feasible_list(
    ...
    for action in [
        ...
        "Slide-PivotF2CW",
        "Slide-PivotF2CCW",
    ]:  # another action: move to target grasp
...
def whether_AdjustGraspAction_feasible(
    ...
    if contact_finger_number == 0:
        ...
        if action == "MoveGripper":
            ...
            if (
                abs(F1_contact_normal[0] + F2_contact_normal[0]) < 0.0001
                and abs(F1_contact_normal[1] + F2_contact_normal[1]) < 0.0001
            ):
                if action == "SlideF1+" or action == "SlideF2-":
                    contact_mode3 = dict(
                        {"F1": "S+", "F2": "S-"}, **dict.fromkeys(contact_index_E, "R")
                    )
                    if whether_stable_simple(
                        contact_index,
                        contact_list,
...
def finger_position_after_action(
    ...
    if (
        ...
        if "F1CCW" in action:
            ...
            effector_trajectory = point_position_after_rotation_batch(
                current_F2,
                current_F1,
                np.arange(
                    1 * pi / 180, target_rotation_angle - 0.2 * pi / 180, 1 * pi / 180
                )
                * 180
                / pi,
            )
        ...
        if (
            MultiPoint(effector_trajectory).distance(Polygon(current_vertex_position))
            == 0
            or MultiPoint(effector_trajectory).distance(LineString(profile_environment))
            < 0.02
        ):
            return []
        cost = (
        ...
        target_gripper_pose_set.append(
            [
                (center_position[0], center_position[1], aperture, angle),
                cost,
                [
                    [
                        "rotate",
        ...
        if action == "SlideF1+" or action == "SlideF1-":
            ...
            if (
                action == "SlideF1+"
                and np.cross(contact_normal_F1, slide_direction) > 0
            ):
            ...
            if (
                action == "SlideF2+"
                and np.cross(contact_normal_F2, slide_direction) > 0
            ):
                slide_direction = [-slide_direction[0], -slide_direction[1]]
            elif (
                action == "SlideF2-"
        ...
        for sliding_length in np.arange(0.05, max_sliding_distance, 0.05):
            target_F1 = [
                current_F1[0] + sliding_length * slide_direction[0],
                current_F1[1] + sliding_length * slide_direction[1],
            ]
            target_F2 = [
                current_F2[0] + sliding_length * slide_direction[0],
                current_F2[1] + sliding_length * slide_direction[1],
            ]
            cost = sliding_length + 1
            if not Feasible_Grasp(
                current_vertex_position, target_F1, target_F2, profile_environment
            ):
                break
            aperture = Point(target_F1).distance(Point(target_F2))
            center_position = [
        ...
        for sliding_length in np.arange(0.05, max_sliding_distance, 0.05):
            ...
            if (
                MultiPoint(effector_trajectory).distance(
                    Polygon(current_vertex_position)
                )
                == 0
                or MultiPoint(effector_trajectory).distance(
