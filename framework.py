...
def object_pose_plan_consider_move_in_air(
    ...
    initial_contact_state = obtain_contact_state(
        initial_object_vertex_position, environment
    )
    target_contact_state = obtain_contact_state(
        target_object_vertex_position, environment
    )
    if initial_contact_state == ["no_contact"]:
        for direction in [[0, -1], [-1, 0], [0, 1], [1, 0]]:
            for i in np.arange(0, 5, 0.01):
                temp_new_vertex_position = Move_in_air_next_vertex_position(
                    initial_object_vertex_position, [direction, i], environment
                )
                new_contact_state = obtain_contact_state(
                    temp_new_vertex_position, environment
                )
    ...
    if target_contact_state == ["no_contact"]:
        for direction in [[0, -1], [-1, 0], [0, 1], [1, 0]]:
            for i in np.arange(0, 5, 0.01):
                temp_new_vertex_position = Move_in_air_next_vertex_position(
                    target_object_vertex_position, [direction, i], environment
                )
                new_contact_state = obtain_contact_state(
                    temp_new_vertex_position, environment
                )
                ...
                continue
            break
    if initial_contact_state != ["no_contact"] and target_contact_state != [
        "no_contact"
    ]:
        action_parameter_sequence, object_pose_trajectory = object_pose_plan(
            initial_object_vertex_position,
            None,
            action_sequence,
            contact_state_trajectory,
        ...
        (
            ...
            last_contact_state_trajectory,
            last_action_parameter_sequence,
            last_object_pose_trajectory,
        ) = object_pose_plan_within_contact_state(
            object_pose_trajectory[-1], target_object_vertex_position, environment
        )
    ...
    elif initial_contact_state == ["no_contact"] and target_contact_state == [
        "no_contact"
    ]:
        action_parameter_sequence, object_pose_trajectory = object_pose_plan(
            object_pose_after_ini,
            None,
            action_sequence,
            contact_state_trajectory,
            environment,
        ...
        (
            ...
            object_pose_trajectory[-1], object_pose_before_end, environment
        )
        action_sequence += last_action_sequence
        contact_state_trajectory += last_contact_state_trajectory
        action_parameter_sequence += last_action_parameter_sequence
        ...
        contact_state_trajectory = (
            [contact_state_after_ini] + contact_state_trajectory + [["no_contact"]]
        )
        ...
        action_parameter_sequence = (
            [action_parameter_after_ini]
            + action_parameter_sequence
            + [action_parameter_before_end]
        )
    elif initial_contact_state == ["no_contact"] and target_contact_state != [
        "no_contact"
    ]:
        ...
        object_pose_trajectory = object_pose_trajectory + [
            target_object_vertex_position
        ]
        action_parameter_sequence = action_parameter_sequence + [
            action_parameter_before_end
        ]
    return (
        action_sequence,
...
def framework(
    ...
    feasible_fcp_set = bottom_layer.feasible_finger_contact_pair_set(
        initial_object_vertex_position
    )
    ...
    while top_layer_iteration < 30:
        if top_layer_iteration != 0:
            ...
            for i in range(len(contact_state_trajectory_temp) - 1):
                for (
                    contact_state_key,
                    contact_state_cost,
                ) in contact_state0.action_cost.items():
                    if tuple(sorted(contact_state_trajectory_temp[i])) == tuple(
                        sorted(contact_state_key)
                    ):
                        for j in range(len(contact_state_cost)):
                            if tuple(
                                sorted(
                                    contact_state0.next_contact_state[
                                        contact_state_key
                                    ][j]
                                )
                            ) == tuple(sorted(contact_state_trajectory_temp[i + 1])):
                                contact_state0.action_cost[contact_state_key][j] += 0.1
        if initial_contact_state != ["no_contact"] and target_contact_state != [
            "no_contact"
        ]:
            action_sequence, contact_state_trajectory = (
                contact_state0.contact_state_plan(
                    initial_contact_state, target_contact_state
                )
            )
        elif initial_contact_state == ["no_contact"] and target_contact_state == [
            "no_contact"
            ...
            action_sequence, contact_state_trajectory = (
                contact_state0.contact_state_plan(
                    contact_state_after_ini, contact_state_before_end
                )
            )
        elif initial_contact_state == ["no_contact"] and target_contact_state != [
            "no_contact"
            ...
            action_sequence, contact_state_trajectory = (
                contact_state0.contact_state_plan(
                    contact_state_after_ini, target_contact_state
                )
            )
        elif initial_contact_state != ["no_contact"] and target_contact_state == [
            "no_contact"
            ...
            action_sequence, contact_state_trajectory = (
                contact_state0.contact_state_plan(
                    initial_contact_state, contact_state_before_end
                )
            )
        if (
            action_sequence in action_sequence_memory
            and contact_state_trajectory in contact_state_trajectory_memory
        ):
            continue
        ...
        while repeating_time_kind_A < 2:  # 4
            ...
            try:
                if repeating_time_kind_B == 0:
                    (
                        ...
                        action_parameter_sequence,
                        object_pose_trajectory,
                    ) = object_pose_plan_consider_move_in_air(
                        initial_object_vertex_position,
                        target_object_vertex_position,
                        action_sequence,
                        contact_state_trajectory,
                        environment,
                        repeating_time_kind_A,
                    )
                    ...
                    old_object_pose_trajectory = object_pose_trajectory[:]
                    action_sequence = (
                        old_action_sequence[0:unsuitable_action_index]
                        + [old_action_sequence[unsuitable_action_index]]
                        + [old_action_sequence[unsuitable_action_index]]
                        + old_action_sequence[(unsuitable_action_index + 1) : :]
                    )
                    contact_state_trajectory = (
                        old_contact_state_trajectory[0:unsuitable_action_index]
                        + [old_contact_state_trajectory[unsuitable_action_index]]
                        + [old_contact_state_trajectory[unsuitable_action_index]]
                        + old_contact_state_trajectory[
                            (unsuitable_action_index + 1) : :
                        ]
                    )
                    ...
                    if old_action_sequence[unsuitable_action_index] == "Tip":
                        action_parameter_sequence = (
                            temp_action_parameter_sequence[0:unsuitable_action_index]
                            + [
                                [
                                    temp_action_parameter_sequence[
                                        unsuitable_action_index
                                    ][0],
                                    temp_action_parameter_sequence[
                                        unsuitable_action_index
                                    ][1],
                        ...
                        object_pose_trajectory = (
                            temp_object_pose_trajectory[0:unsuitable_action_index]
                            + [
                                Tip_next_vertex_position(
                                    temp_object_pose_trajectory[
                                        unsuitable_action_index - 1
                                    ],
                                    action_parameter_sequence[unsuitable_action_index],
                                    environment,
                                )
                            ]
                            + temp_object_pose_trajectory[(unsuitable_action_index)::]
                        )
                        ...
                        old_action_sequence[unsuitable_action_index] == "Tilting-slide"
                    ):
                        action_parameter_sequence = (
                            temp_action_parameter_sequence[0:unsuitable_action_index]
                            + [
                        ...
                        object_pose_trajectory = (
                            temp_object_pose_trajectory[0:unsuitable_action_index]
                            + [
                                Tilting_slide_next_vertex_position(
                                    temp_object_pose_trajectory[
                                        unsuitable_action_index - 1
                                    ],
                                    action_parameter_sequence[unsuitable_action_index],
            ...
            ) = bottom_layer.obtain_feasible_fcp_set_of_each_pose(
                feasible_fcp_set,
                object_pose_trajectory_plus_ini,
            ...
            suitable_fcp_list_sequence, suitable_fcp_cost_list_sequence = (
                bottom_layer.obtain_suitable_fcp_set_of_each_action(
                    action_sequence,
                    action_parameter_sequence,
                    object_pose_trajectory_plus_ini,
                    feasible_fcp_set_list,
            ...
            for j in unsuitable_action_index_set:
                if action_sequence[j] == "Move-in-air" or action_sequence[j] == "Push":
                    ...
                    break
            if unsuitable_grasp_dichotomy_not_need:
                repeating_time_kind_B = 0
                repeating_time_kind_A += 1
                continue
            ...
            gripper_pose_map_moveobject_link_set = (
                bottom_layer.gripper_pose_map_moveobject(
                    action_sequence,
                    action_parameter_sequence,
                    object_pose_trajectory_plus_ini,
            ...
            gripper_pose_map_adjustgrasp_link_set = (
                bottom_layer.gripper_pose_map_adjustgrasp(
                    object_pose_trajectory_plus_ini,
                    feasible_fcp_set_list,
                    feasible_gripper_pose_set_list,
                    gripper_pose_map_moveobject_link_set,
            ...
            result_temp = bottom_layer.A_star_find_action_path(
                ini_gripper_pose,
                gripper_pose_map_moveobject_link_set,
                gripper_pose_map_adjustgrasp_link_set,
                object_pose_trajectory_plus_ini,
            ...
            if result_temp != False:
                gripper_trajectory_node_sequence, action_and_parameter_sequence = (
                    result_temp
                )
                ...
                for i in range(len(action_and_parameter_sequence)):
                    if action_and_parameter_sequence[i][0] == "MoveObject":
                        ...
                        object_pose_trajectory += (
                            bottom_layer.object_trajectory_moveobject(
                                old_object_pose,
                                action_and_parameter_sequence[i][1],
                                action_and_parameter_sequence[i][2],
                                environment,
                            )
                        )
                        gripper_pose_trajectory += (
                            bottom_layer.gripper_trajectory_moveobject(
                                old_object_pose,
                                gripper_pose_trajectory[-1],
                                action_and_parameter_sequence[i][1],
                                action_and_parameter_sequence[i][2],
                                environment,
                            )
                        ...
                        if action_and_parameter_sequence[i][1][0] == "MoveGripper":
                            ...
                            gripper_pose_trajectory.append(
                                action_and_parameter_sequence[i][1][1]
                            )
                            print(
                                object_pose_trajectory[-1], gripper_pose_trajectory[-1]
                            )
                        else:
                            new_gripper_trajectory = bottom_layer.action_trajectory(
                                gripper_pose_trajectory[-1],
                                action_and_parameter_sequence[i][1],
                            )
                            object_pose_trajectory += [
                                object_pose_trajectory[-1]
                            ] * len(new_gripper_trajectory)
