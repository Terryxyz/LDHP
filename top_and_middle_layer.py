...
def obtain_contact_state(current_vertex_position, profile_environment, current_vertex_position_buffer=None, accuracy = 0.004):
    ...
    for i in range(len(current_vertex_position)):
        for E_key in E:
            ...
            if abs(np.cross(generate_vector(V[i], V[(i+1)%(len(current_vertex_position))]), generate_vector(E[E_key][0], E[E_key][1]))/(LineString([V[i], V[(i+1)%(len(current_vertex_position))]]).length*LineString(E[E_key]).length))>0.017:
                continue
            if Point(V[i]).distance(LineString(E[E_key]))<accuracy and Point(V[(i+1)%(len(current_vertex_position))]).distance(LineString(E[E_key]))<accuracy:
                ...
                temp_point0 = [V[i][0]+(V[(i+1)%(len(current_vertex_position))][0]-V[i][0])/10*3, V[i][1]+(V[(i+1)%(len(current_vertex_position))][1]-V[i][1])/10*2]
                if Point(V[i]).distance(LineString(E[E_key]))<accuracy and Point(temp_point0).distance(LineString(E[E_key]))<accuracy:
                    contact_state_list.append('l'+str(i+1)+'E'+str(E_key))
                else:
                    temp_point1 = [V[i][0]+(V[(i+1)%(len(current_vertex_position))][0]-V[i][0])/10*8, V[i][1]+(V[(i+1)%(len(current_vertex_position))][1]-V[i][1])/10*8] 
...
def whether_contains(principle_contact_state1, principle_contact_state2, object_edge_number):
    ...
    if principle_contact_state1 == principle_contact_state2:
        return False
    elif principle_contact_state1[principle_contact_state1.index('E')::] != principle_contact_state2[principle_contact_state2.index('E')::]:
        return False
        ...
        return False
    elif (int(principle_contact_state1[1:principle_contact_state1.index('E')])%object_edge_number+1)==int(principle_contact_state2[1:principle_contact_state2.index('E')]):
        return True
    elif int(principle_contact_state1[1:principle_contact_state1.index('E')])==int(principle_contact_state2[1:principle_contact_state2.index('E')]):
        return True
    else:
        return False
        ...
        for principle_contact in temp_contact_state2:
            for principle_contact0 in temp_contact_state2:
                if principle_contact0==principle_contact:
                    continue
                if principle_contact in contain_list and principle_contact0 in contain_list and set(contain_list[principle_contact]).intersection(set(contain_list[principle_contact0]))!=set():
                    return False
        return True
    return False
...
def finding_configuration_cos_contact_state(current_object_vertex, environment, target_contact_state, scope = [[-0.1,-0.1], [3.1,-0.1], [3.1,3.1], [-0.1,3.1]]):
    ...
    if target_contact_state == ['Invalid'] or target_contact_state == ['no_contact']:
        ...
        if temp_target_contact_state[0] == 'l':
            mating_edge_index = int(temp_target_contact_state[1:temp_target_contact_state.index('E')])
            mating_environment_index = int(temp_target_contact_state[temp_target_contact_state.index('E')+1::])
            mating_edge = [current_object_vertex[mating_edge_index-1], current_object_vertex[mating_edge_index % len(current_object_vertex)]]
            mating_environment_edge = [environment[len(environment)-mating_environment_index-1], environment[len(environment)-mating_environment_index]]
            rotate_angle = calculate_intersect_angle_AB_CD(mating_edge[0],mating_edge[1],mating_environment_edge[0],mating_environment_edge[1])
            object_vertex_position_after_rotation = [point_position_after_rotation(vertex, current_object_vertex[0], rotate_angle) for vertex in current_object_vertex]
            mating_edge = [object_vertex_position_after_rotation[mating_edge_index-1], object_vertex_position_after_rotation[mating_edge_index % len(current_object_vertex)]]
            translating_distance = abs(CtoAB(mating_environment_edge[0],mating_environment_edge[1],mating_edge[0]))
            translating_direction = generate_vector(mating_edge[0], mating_edge[1])
            translating_direction = [translating_direction[1]/LineString(mating_edge).length, translating_direction[0]/LineString(mating_edge).length]
            if np.dot(translating_direction, generate_vector(mating_environment_edge[0], mating_edge[0]))>0:
                translating_direction = [-translating_direction[0], -translating_direction[1]]
            object_vertex_position_after_translation = [[vertex[0]+translating_distance*translating_direction[0], vertex[1]+translating_distance*translating_direction[1]] for vertex in object_vertex_position_after_rotation]
            if sorted(obtain_contact_state(object_vertex_position_after_translation, environment))==sorted(target_contact_state):
                if Polygon(object_vertex_position_after_translation).within(Polygon(scope))==True:
                    return object_vertex_position_after_translation
            mating_edge = [object_vertex_position_after_translation[mating_edge_index-1], object_vertex_position_after_translation[mating_edge_index % len(current_object_vertex)]]
            translating_distance = LineString(mating_edge).distance(LineString(mating_environment_edge))
            mating_edge_direction = generate_vector(mating_edge[0], mating_edge[1])
            translating_direction = [mating_edge_direction[0]/LineString(mating_edge).length, mating_edge_direction[1]/LineString(mating_edge).length]
            object_vertex_position_after_translation = [[vertex[0]+translating_distance*translating_direction[0], vertex[1]+translating_distance*translating_direction[1]] for vertex in object_vertex_position_after_translation]
            mating_edge = [object_vertex_position_after_translation[mating_edge_index-1], object_vertex_position_after_translation[mating_edge_index % len(current_object_vertex)]]
            if LineString(mating_edge).distance(LineString(mating_environment_edge)) != 0:
                translating_direction = [-translating_direction[0], -translating_direction[1]]
                object_vertex_position_after_translation = [[vertex[0]+2*translating_distance*translating_direction[0], vertex[1]+2*translating_distance*translating_direction[1]] for vertex in object_vertex_position_after_translation]
            object_vertex_position_before_translation=object_vertex_position_after_translation[:]
            mating_edge = [object_vertex_position_before_translation[mating_edge_index-1], object_vertex_position_before_translation[mating_edge_index % len(current_object_vertex)]]
            for translation_step in np.append(np.arange(0, -LineString(mating_edge).length-LineString(mating_environment_edge).length, -(LineString(mating_edge).length+LineString(mating_environment_edge).length)/40),np.arange(0, LineString(mating_edge).length+LineString(mating_environment_edge).length, (LineString(mating_edge).length+LineString(mating_environment_edge).length)/40)):
                object_vertex_position_after_translation = [[vertex[0]+translation_step*translating_direction[0], vertex[1]+translation_step*translating_direction[1]] for vertex in object_vertex_position_before_translation]
                if sorted(obtain_contact_state(object_vertex_position_after_translation, environment))==sorted(target_contact_state):
                    if Polygon(object_vertex_position_after_translation).within(Polygon(scope))==True:
                        return object_vertex_position_after_translation
            else:
                object_vertex_position_before_translation = [point_position_after_rotation(vertex, mating_edge[0], 180) for vertex in object_vertex_position_before_translation]
                for translation_step in np.append(np.arange(0, -LineString(mating_edge).length-LineString(mating_environment_edge).length, -(LineString(mating_edge).length+LineString(mating_environment_edge).length)/15),np.arange(0, LineString(mating_edge).length+LineString(mating_environment_edge).length, (LineString(mating_edge).length+LineString(mating_environment_edge).length)/15)):
                    object_vertex_position_after_translation = [[vertex[0]+translation_step*translating_direction[0], vertex[1]+translation_step*translating_direction[1]] for vertex in object_vertex_position_before_translation]
                    if sorted(obtain_contact_state(object_vertex_position_after_translation, environment))==sorted(target_contact_state):
                        if Polygon(object_vertex_position_after_translation).within(Polygon(scope))==True:
                            return object_vertex_position_after_translation
        if temp_target_contact_state[0] == 'V':
            mating_vertex_index = int(temp_target_contact_state[1:temp_target_contact_state.index('E')])
            mating_environment_index = int(temp_target_contact_state[temp_target_contact_state.index('E')+1::])
            mating_vertex = current_object_vertex[mating_vertex_index-1]
            mating_environment_edge = [environment[len(environment)-mating_environment_index-1], environment[len(environment)-mating_environment_index]]
            mating_environment_edge_midpoint = [(mating_environment_edge[0][0]+mating_environment_edge[1][0])/2.0, (mating_environment_edge[0][1]+mating_environment_edge[1][1])/2.0]
            translatig_direction_and_distance = generate_vector(mating_vertex, mating_environment_edge_midpoint)
            object_vertex_position_after_translation = [[vertex[0]+translatig_direction_and_distance[0], vertex[1]+translatig_direction_and_distance[1]] for vertex in current_object_vertex]
            mating_vertex = object_vertex_position_after_translation[mating_vertex_index-1]
            for rotation_step in np.append(np.arange(0, 180, 5), np.arange(0, -180, -5)):
                object_vertex_position_after_rotation = [point_position_after_rotation(vertex, mating_vertex, rotation_step) for vertex in object_vertex_position_after_translation]
                if sorted(obtain_contact_state(object_vertex_position_after_rotation, environment))==sorted(target_contact_state):
                    if Polygon(object_vertex_position_after_rotation).within(Polygon(scope))==True:
        ...
        if temp_target_contact_state0==temp_target_contact_state1:
            ...
            return False
        elif whether_contains(temp_target_contact_state0, temp_target_contact_state1, len(current_object_vertex))==True or whether_contains(temp_target_contact_state1, temp_target_contact_state0, len(current_object_vertex))==True:

            return False
        if temp_target_contact_state0[temp_target_contact_state0.index('E')+1::] == temp_target_contact_state1[temp_target_contact_state1.index('E')+1::]:

            if ((temp_target_contact_state0[0]=='V' and temp_target_contact_state1[0]=='V') or (temp_target_contact_state0[0]=='l' and temp_target_contact_state1[0]=='l')) and (int(temp_target_contact_state0[1:temp_target_contact_state0.index('E')])%len(current_object_vertex)+1==int(temp_target_contact_state1[1:temp_target_contact_state1.index('E')]) or int(temp_target_contact_state1[1:temp_target_contact_state1.index('E')])%len(current_object_vertex)+1==int(temp_target_contact_state0[1:temp_target_contact_state0.index('E')])):

                return False
            if temp_target_contact_state0[0]=='l' and temp_target_contact_state1[0]=='l':

                mating_edge_index0 = int(temp_target_contact_state0[1:temp_target_contact_state0.index('E')])
                mating_edge_index1 = int(temp_target_contact_state1[1:temp_target_contact_state1.index('E')])
                mating_edge0 = [current_object_vertex[mating_edge_index0-1], current_object_vertex[mating_edge_index0 % len(current_object_vertex)]]
                mating_edge1 = [current_object_vertex[mating_edge_index1-1], current_object_vertex[mating_edge_index1 % len(current_object_vertex)]]
                mating_edge0_edge1_link = [mating_edge0[1], mating_edge1[0]]
                if abs(np.cross(generate_vector(mating_edge0[0], mating_edge0[1]), generate_vector(mating_edge0_edge1_link[0], mating_edge0_edge1_link[1])))/(LineString(mating_edge0).length*LineString(mating_edge0_edge1_link).length) > 0.017:
                    return False
                if abs(np.cross(generate_vector(mating_edge1[0], mating_edge1[1]), generate_vector(mating_edge0_edge1_link[0], mating_edge0_edge1_link[1])))/(LineString(mating_edge1).length*LineString(mating_edge0_edge1_link).length) > 0.017:
                    return False
                if mating_edge_index0<mating_edge_index1:
                    bounded_object_vertex = current_object_vertex[0:max(0, mating_edge_index0-1)]+[current_object_vertex[mating_edge_index0-1], current_object_vertex[mating_edge_index1]]+current_object_vertex[(mating_edge_index1+1)::]
                    mating_index_new = mating_edge_index0
                else:
                    bounded_object_vertex = current_object_vertex[0:max(0, mating_edge_index1-1)]+[current_object_vertex[mating_edge_index1-1], current_object_vertex[mating_edge_index0]]+current_object_vertex[(mating_edge_index0+1)::]
                    mating_index_new = mating_edge_index1
            elif temp_target_contact_state0[0]=='V' and temp_target_contact_state1[0]=='V':

                if finding_configuration_cos_contact_state(current_object_vertex, environment, ['l'+temp_target_contact_state0[1::], temp_target_contact_state1, 'double_contact'], scope)!=False:
                    return False
                elif finding_configuration_cos_contact_state(current_object_vertex, environment, ['l'+str((int(temp_target_contact_state0[1:temp_target_contact_state0.index('E')])-2)%len(current_object_vertex)+1)+temp_target_contact_state0[temp_target_contact_state0.index('E')::], temp_target_contact_state1, 'double_contact'], scope)!=False:
                    return False
                elif finding_configuration_cos_contact_state(current_object_vertex, environment, ['l'+temp_target_contact_state1[1::], temp_target_contact_state0, 'double_contact'], scope)!=False:
                    return False
                elif finding_configuration_cos_contact_state(current_object_vertex, environment, ['l'+str((int(temp_target_contact_state1[1:temp_target_contact_state1.index('E')])-2)%len(current_object_vertex)+1)+temp_target_contact_state1[temp_target_contact_state1.index('E')::], temp_target_contact_state0, 'double_contact'], scope)!=False:
                    return False
                mating_vertex_index0 = int(temp_target_contact_state0[1:temp_target_contact_state0.index('E')])
                mating_vertex_index1 = int(temp_target_contact_state1[1:temp_target_contact_state1.index('E')])
                ...
                if mating_vertex_index0<mating_vertex_index1:
                    bounded_object_vertex = current_object_vertex[0:max(0, mating_vertex_index0-1)]+[current_object_vertex[mating_vertex_index0-1], current_object_vertex[mating_vertex_index1-1]]+current_object_vertex[mating_vertex_index1::]
                    mating_index_new = mating_vertex_index0
                else:
                    bounded_object_vertex = current_object_vertex[0:max(0, mating_vertex_index1-1)]+[current_object_vertex[mating_vertex_index1-1], current_object_vertex[mating_vertex_index0-1]]+current_object_vertex[mating_vertex_index0::]
                    mating_index_new = mating_vertex_index1
            else:

                if temp_target_contact_state0[0]=='l' and temp_target_contact_state1[0]=='V':
                ...
                if abs(np.cross(generate_vector(mating_edge1[0], mating_edge1[1]), generate_vector(mating_vertex0_edge1_link[0], mating_vertex0_edge1_link[1])))/(LineString(mating_edge1).length*LineString(mating_vertex0_edge1_link).length) > 0.017:
                    return False
                if mating_vertex_index0<mating_edge_index1:
                    bounded_object_vertex = current_object_vertex[0:max(0, mating_vertex_index0-1)]+[current_object_vertex[mating_vertex_index0-1], current_object_vertex[mating_edge_index1]]+current_object_vertex[(mating_edge_index1+1)::]
                    mating_index_new = mating_vertex_index0
                else:
                    bounded_object_vertex = current_object_vertex[0:max(0, mating_edge_index1-1)]+[current_object_vertex[mating_edge_index1-1], current_object_vertex[mating_vertex_index0-1]]+current_object_vertex[(mating_vertex_index0)::]
                    mating_index_new = mating_edge_index1
            bounded_object_vertex_after_moving = finding_configuration_cos_contact_state(bounded_object_vertex, environment, ['l'+str(mating_index_new)+'E'+temp_target_contact_state0[temp_target_contact_state0.index('E')+1::], 'single_contact'], scope)   
            if bounded_object_vertex_after_moving==False:
                return False
            mating_edge_new = [bounded_object_vertex_after_moving[mating_index_new-1], bounded_object_vertex_after_moving[mating_index_new]]
            mating_environment_index = int(temp_target_contact_state1[temp_target_contact_state1.index('E')+1::])
            mating_environment_edge = [environment[len(environment)-mating_environment_index-1], environment[len(environment)-mating_environment_index]]
            mating_environment_edge_direction = [(mating_environment_edge[1][0]-mating_environment_edge[0][0])/LineString(mating_environment_edge).length, (mating_environment_edge[1][1]-mating_environment_edge[0][1])/LineString(mating_environment_edge).length]
            for translating_step in np.append(np.arange(0, -LineString(mating_environment_edge).length, -LineString(mating_environment_edge).length/15.0), np.arange(0, LineString(mating_environment_edge).length, LineString(mating_environment_edge).length/15.0)):
                bounded_object_vertex_after_translation = [[vertex[0]+translating_step*mating_environment_edge_direction[0], vertex[1]+translating_step*mating_environment_edge_direction[1]] for vertex in bounded_object_vertex_after_moving]
                if LineString([bounded_object_vertex_after_translation[mating_index_new-1], bounded_object_vertex_after_translation[mating_index_new]]).within(LineString(mating_environment_edge).buffer(0.005)):
                    delta_translate, _, A_rad=calculate_parameter_translate_and_rotate_moveA(bounded_object_vertex[0], bounded_object_vertex[1], bounded_object_vertex_after_translation[0], bounded_object_vertex_after_translation[1])
                    object_vertex_position_after_translation = [[vertex[0]+delta_translate[0], vertex[1]+delta_translate[1]] for vertex in current_object_vertex]
                    object_vertex_position_after_rotation = [point_position_after_rotation(vertex, object_vertex_position_after_translation[0], A_rad*180/pi) for vertex in object_vertex_position_after_translation]
                    if sorted(obtain_contact_state(object_vertex_position_after_rotation, environment))==sorted(target_contact_state):
                        return object_vertex_position_after_rotation
        elif temp_target_contact_state0[0]=='l' and temp_target_contact_state1[0]=='l':
            object_vertex_position_after_one_mate = finding_configuration_cos_contact_state(current_object_vertex, environment, [temp_target_contact_state0, 'single_contact'], scope)
            if object_vertex_position_after_one_mate==False:
                return False
            mating_edge_index = int(temp_target_contact_state1[1:temp_target_contact_state1.index('E')])
            mating_environment_index = int(temp_target_contact_state1[temp_target_contact_state1.index('E')+1::])
            mating_edge = [object_vertex_position_after_one_mate[mating_edge_index-1], object_vertex_position_after_one_mate[mating_edge_index % len(current_object_vertex)]]
            mating_environment_edge = [environment[len(environment)-mating_environment_index-1], environment[len(environment)-mating_environment_index]]
            if abs(np.cross(generate_vector(mating_edge[0], mating_edge[1]), generate_vector(mating_environment_edge[0], mating_environment_edge[1])))/(LineString(mating_edge).length*LineString(mating_environment_edge).length)>0.017:
                return False
            translating_distance = abs(CtoAB(mating_environment_edge[0],mating_environment_edge[1],mating_edge[0]))
            translating_environment_index = int(temp_target_contact_state0[temp_target_contact_state0.index('E')+1::])
            translating_environment_edge = [environment[len(environment)-translating_environment_index-1], environment[len(environment)-translating_environment_index]]
            translating_direction = generate_vector(translating_environment_edge[0], translating_environment_edge[1])
            translating_direction = [translating_direction[0]/LineString(translating_environment_edge).length, translating_direction[1]/LineString(translating_environment_edge).length]
            object_vertex_position_after_translation = [[vertex[0]+translating_direction[0]*translating_distance, vertex[1]+translating_direction[1]*translating_distance] for vertex in object_vertex_position_after_one_mate]
            if sorted(obtain_contact_state(object_vertex_position_after_translation, environment))==sorted(target_contact_state):
                if Polygon(object_vertex_position_after_translation).within(Polygon(scope))==True:
                    return object_vertex_position_after_translation
            object_vertex_position_after_translation = [[vertex[0]-translating_direction[0]*translating_distance, vertex[1]-translating_direction[1]*translating_distance] for vertex in object_vertex_position_after_one_mate]
            ...
            if temp_target_contact_state0[0]=='V' and temp_target_contact_state1[0]=='l':
                temp = temp_target_contact_state0[:]
                temp_target_contact_state0 = temp_target_contact_state1[:]
                temp_target_contact_state1 = temp[:]
            object_vertex_position_after_one_mate = finding_configuration_cos_contact_state(current_object_vertex, environment, [temp_target_contact_state0, 'single_contact'], scope)
            ...
            rotate_time = 0 
            while True:
                mating_vertex_index = int(temp_target_contact_state1[1:temp_target_contact_state1.index('E')])
                mating_environment_index = int(temp_target_contact_state1[temp_target_contact_state1.index('E')+1::])    
                mating_vertex = object_vertex_position_after_one_mate[mating_vertex_index-1]
                mating_environment_edge = [environment[len(environment)-mating_environment_index-1], environment[len(environment)-mating_environment_index]]
                translating_environment_index = int(temp_target_contact_state0[temp_target_contact_state0.index('E')+1::])
                translating_environment_edge = [environment[len(environment)-translating_environment_index-1], environment[len(environment)-translating_environment_index]]
                translating_direction = generate_vector(translating_environment_edge[0], translating_environment_edge[1])
                translating_direction = [translating_direction[0]/LineString(translating_environment_edge).length, translating_direction[1]/LineString(translating_environment_edge).length]
                intersect_point = calculate_intersect_point_AB_CD(mating_environment_edge[0], mating_environment_edge[1], mating_vertex, [mating_vertex[0]+translating_direction[0], mating_vertex[1]+translating_direction[1]])
                try:
                    translating_distance = LineString([intersect_point, mating_vertex]).length
                except:
                    break
                object_vertex_position_after_translation = [[vertex[0]+translating_direction[0]*translating_distance, vertex[1]+translating_direction[1]*translating_distance] for vertex in object_vertex_position_after_one_mate]
                if sorted(obtain_contact_state(object_vertex_position_after_translation, environment))==sorted(target_contact_state):
                ...
                if sorted(obtain_contact_state(object_vertex_position_after_translation, environment))==sorted(target_contact_state):
                    if Polygon(object_vertex_position_after_translation).within(Polygon(scope))==True:
                        return object_vertex_position_after_translation 
                if temp_target_contact_state0[0]=='V' and temp_target_contact_state1[0]=='V' and rotate_time < 360.0/1.0:
                    rotate_time += 1
                    object_vertex_position_after_one_mate = [point_position_after_rotation(vertex, object_vertex_position_after_one_mate[int(temp_target_contact_state0[1:temp_target_contact_state0.index('E')])-1], 1) for vertex in object_vertex_position_after_one_mate]       
                else:
                    break           
    return False
...
def Tilting_slide_contact_edge_point(current_vertex_position, target_contact_state, profile_environment):
    ...
    for contact_state_element in current_contact_state:
        if re.match('V', contact_state_element)!=None:
            contact_point = current_vertex_position[int(contact_state_element[1:contact_state_element.index('E')])-1]
            for i in range(len(profile_environment)-1):
                if contact_state_element[contact_state_element.index('E')+1::]==str(i+1):
                    contact_edge['E'+str(i+1)] = E[i+1]
                    contact_edge_point['E'+str(i+1)] = contact_point
        else:
            contact_point = [current_vertex_position[int(contact_state_element[1:contact_state_element.index('E')])-1], current_vertex_position[int(contact_state_element[1:contact_state_element.index('E')])%(len(current_vertex_position))]]
...
def Tilting_slide_calculate_next_vertex_position_simplified(contact_edge_point, total_angle, old_vertex_position, profile_environment):
    ...
    delta_translate, D_r, A_r = calculate_parameter_translate_and_rotate_moveA(contact_edge_point[E_key_set[0]], contact_edge_point[E_key_set[1]], new_Ea_contact_point, new_Eb_contact_point)
    A_r = A_r*180/pi
    ...
    def contact_state_map(self, environment, initial_vertex_position, scope = [[-0.1,-0.1], [3.1,-0.1], [3.1,3.1], [-0.1,3.1]]):
        ...
        for edge_index in range(1, object_edge_number+1):
            for environment_edge_index in range(1, environment_edge_number+1):
                if finding_configuration_cos_contact_state(initial_vertex_position, environment, ['l'+str(edge_index)+'E'+str(environment_edge_index), 'single_contact'], scope) != False:
                    potential_contact_state_set.append(sorted(['l'+str(edge_index)+'E'+str(environment_edge_index), 'single_contact']))
                if finding_configuration_cos_contact_state(initial_vertex_position, environment, ['V'+str(edge_index)+'E'+str(environment_edge_index), 'single_contact'], scope) != False:
                    potential_contact_state_set.append(sorted(['V'+str(edge_index)+'E'+str(environment_edge_index), 'single_contact']))
                for edge_index2 in range(1, object_edge_number+1):
                    for environment_edge_index2 in range(1, environment_edge_number+1):
                        ...
                        if sorted(['l'+str(edge_index)+'E'+str(environment_edge_index), 'l'+str(edge_index2)+'E'+str(environment_edge_index2), 'double_contact']) not in potential_contact_state_set and finding_configuration_cos_contact_state(initial_vertex_position, environment, ['l'+str(edge_index)+'E'+str(environment_edge_index), 'l'+str(edge_index2)+'E'+str(environment_edge_index2), 'double_contact'], scope) != False:
                            potential_contact_state_set.append(sorted(['l'+str(edge_index)+'E'+str(environment_edge_index), 'l'+str(edge_index2)+'E'+str(environment_edge_index2), 'double_contact']))
                        #print 'bbb', ['l'+str(edge_index)+'E'+str(environment_edge_index), 'V'+str(edge_index2)+'E'+str(environment_edge_index2), 'double_contact']
                        if sorted(['l'+str(edge_index)+'E'+str(environment_edge_index), 'V'+str(edge_index2)+'E'+str(environment_edge_index2), 'double_contact']) not in potential_contact_state_set and finding_configuration_cos_contact_state(initial_vertex_position, environment, ['l'+str(edge_index)+'E'+str(environment_edge_index), 'V'+str(edge_index2)+'E'+str(environment_edge_index2), 'double_contact'], scope) != False:
                            potential_contact_state_set.append(sorted(['l'+str(edge_index)+'E'+str(environment_edge_index), 'V'+str(edge_index2)+'E'+str(environment_edge_index2), 'double_contact']))
                        ...
                        if sorted(['V'+str(edge_index)+'E'+str(environment_edge_index), 'V'+str(edge_index2)+'E'+str(environment_edge_index2), 'double_contact']) not in potential_contact_state_set and finding_configuration_cos_contact_state(initial_vertex_position, environment, ['V'+str(edge_index)+'E'+str(environment_edge_index), 'V'+str(edge_index2)+'E'+str(environment_edge_index2), 'double_contact'], scope) != False:
                            potential_contact_state_set.append(sorted(['V'+str(edge_index)+'E'+str(environment_edge_index), 'V'+str(edge_index2)+'E'+str(environment_edge_index2), 'double_contact'])) 
        ...
        for k in range(len(potential_contact_state_set)):
            if 'double_contact' in potential_contact_state_set[k]:
                potential_contact_state_set_double_first.append(potential_contact_state_set[k])
            else:
                potential_contact_state_set_single_last.append(potential_contact_state_set[k])
        potential_contact_state_set=potential_contact_state_set_double_first+potential_contact_state_set_single_last  
        ...
        for i in range(len(potential_contact_state_set)):
            for j in range(len(potential_contact_state_set)):
                if i==j:
                    continue
                if whether_less_constraint(potential_contact_state_set[i], potential_contact_state_set[j], len(initial_vertex_position))==False and whether_less_constraint(potential_contact_state_set[j], potential_contact_state_set[i], len(initial_vertex_position))==False:
                    continue
                if 'double_contact' in potential_contact_state_set[i] and 'double_contact' in potential_contact_state_set[j]:
                    ...
                    if temp_potential_contact_state_set_i[0][0:temp_potential_contact_state_set_i[0].index('E')]==temp_potential_contact_state_set_i[1][0:temp_potential_contact_state_set_i[1].index('E')] or temp_potential_contact_state_set_j[0][0:temp_potential_contact_state_set_j[0].index('E')]==temp_potential_contact_state_set_j[1][0:temp_potential_contact_state_set_j[1].index('E')]:
                        if tuple(potential_contact_state_set[i]) in next_contact_state:
                            already_have = False
                            for k0 in range(len(next_contact_state[tuple(potential_contact_state_set[i])])):
                                if next_contact_state[tuple(potential_contact_state_set[i])][k0]==tuple(potential_contact_state_set[j]) and action_to_next_contact_state[tuple(potential_contact_state_set[i])][k0]=='Tip':
                                    already_have = True
                                    break
                            if already_have == True:
                                continue
                            next_contact_state[tuple(potential_contact_state_set[i])].append(tuple(potential_contact_state_set[j]))
                            action_to_next_contact_state[tuple(potential_contact_state_set[i])].append('Tip')
                            action_cost[tuple(potential_contact_state_set[i])].append(1)
                            if tuple(potential_contact_state_set[j]) in next_contact_state:
                                next_contact_state[tuple(potential_contact_state_set[j])].append(tuple(potential_contact_state_set[i]))  
                                action_to_next_contact_state[tuple(potential_contact_state_set[j])].append('Tip')
                                action_cost[tuple(potential_contact_state_set[j])].append(1)
                            else:
                                next_contact_state[tuple(potential_contact_state_set[j])]=[tuple(potential_contact_state_set[i])] 
                                action_to_next_contact_state[tuple(potential_contact_state_set[j])]=['Tip']
                                action_cost[tuple(potential_contact_state_set[j])]=[1] 
                        else:
                            next_contact_state[tuple(potential_contact_state_set[i])]=[tuple(potential_contact_state_set[j])]
                            action_to_next_contact_state[tuple(potential_contact_state_set[i])]=['Tip']
                        ...
                        if tuple(potential_contact_state_set[i]) in next_contact_state:
                            already_have = False
                            for k0 in range(len(next_contact_state[tuple(potential_contact_state_set[i])])):
                                if next_contact_state[tuple(potential_contact_state_set[i])][k0]==tuple(potential_contact_state_set[j]) and action_to_next_contact_state[tuple(potential_contact_state_set[i])][k0]=='Tilting-slide':
                                    already_have = True
                                    break
                            if already_have == True:
                                continue
                            next_contact_state[tuple(potential_contact_state_set[i])].append(tuple(potential_contact_state_set[j]))
                            action_to_next_contact_state[tuple(potential_contact_state_set[i])].append('Tilting-slide')
                            action_cost[tuple(potential_contact_state_set[i])].append(1)
                            if tuple(potential_contact_state_set[j]) in next_contact_state:
                                next_contact_state[tuple(potential_contact_state_set[j])].append(tuple(potential_contact_state_set[i]))  
                                action_to_next_contact_state[tuple(potential_contact_state_set[j])].append('Tilting-slide')
                                action_cost[tuple(potential_contact_state_set[j])].append(1)
                            else:
                                next_contact_state[tuple(potential_contact_state_set[j])]=[tuple(potential_contact_state_set[i])] 
                                action_to_next_contact_state[tuple(potential_contact_state_set[j])]=['Tilting-slide']
                                action_cost[tuple(potential_contact_state_set[j])]=[1] 
                        else:
                            next_contact_state[tuple(potential_contact_state_set[i])]=[tuple(potential_contact_state_set[j])]
                            action_to_next_contact_state[tuple(potential_contact_state_set[i])]=['Tilting-slide']
                            action_cost[tuple(potential_contact_state_set[i])]=[1]
                            if tuple(potential_contact_state_set[j]) in next_contact_state:
                                next_contact_state[tuple(potential_contact_state_set[j])].append(tuple(potential_contact_state_set[i]))  
                                action_to_next_contact_state[tuple(potential_contact_state_set[j])].append('Tilting-slide')
                                ...
                                action_to_next_contact_state[tuple(potential_contact_state_set[j])]=['Tilting-slide']
                                action_cost[tuple(potential_contact_state_set[j])]=[1]          
                elif 'single_contact' in potential_contact_state_set[i] and 'single_contact' in potential_contact_state_set[j]:
                    if tuple(potential_contact_state_set[i]) in next_contact_state:
                        already_have = False
                        for k0 in range(len(next_contact_state[tuple(potential_contact_state_set[i])])):
                            if next_contact_state[tuple(potential_contact_state_set[i])][k0]==tuple(potential_contact_state_set[j]) and action_to_next_contact_state[tuple(potential_contact_state_set[i])][k0]=='Tip':
                    ...
                    two_contact_State_intersection = list(set(potential_contact_state_set[i]).intersection(set(potential_contact_state_set[j])))
                    if two_contact_State_intersection != []: 
                        if tuple(potential_contact_state_set[i]) in next_contact_state:
                            already_have = False
                            for k0 in range(len(next_contact_state[tuple(potential_contact_state_set[i])])):
                                if next_contact_state[tuple(potential_contact_state_set[i])][k0]==tuple(potential_contact_state_set[j]) and action_to_next_contact_state[tuple(potential_contact_state_set[i])][k0]=='Push':
                                    already_have = True
                                    break
                            if already_have == True:
                                continue
                            next_contact_state[tuple(potential_contact_state_set[i])].append(tuple(potential_contact_state_set[j]))
                            action_to_next_contact_state[tuple(potential_contact_state_set[i])].append('Push')
                            action_cost[tuple(potential_contact_state_set[i])].append(1)
                            if tuple(potential_contact_state_set[j]) in next_contact_state:
                                next_contact_state[tuple(potential_contact_state_set[j])].append(tuple(potential_contact_state_set[i]))  
                                action_to_next_contact_state[tuple(potential_contact_state_set[j])].append('Push')
                                action_cost[tuple(potential_contact_state_set[j])].append(1)
                            else:
                                next_contact_state[tuple(potential_contact_state_set[j])]=[tuple(potential_contact_state_set[i])] 
                                action_to_next_contact_state[tuple(potential_contact_state_set[j])]=['Push']
                                action_cost[tuple(potential_contact_state_set[j])]=[1] 
                        else:
                            next_contact_state[tuple(potential_contact_state_set[i])]=[tuple(potential_contact_state_set[j])]
                            action_to_next_contact_state[tuple(potential_contact_state_set[i])]=['Push']
                            action_cost[tuple(potential_contact_state_set[i])]=[1]
                            if tuple(potential_contact_state_set[j]) in next_contact_state:
                                next_contact_state[tuple(potential_contact_state_set[j])].append(tuple(potential_contact_state_set[i]))  
                                action_to_next_contact_state[tuple(potential_contact_state_set[j])].append('Push')
                    ...
                    if 'double_contact' in potential_contact_state_set[j]:
                        ...
                        double_contact_contact_state.remove('double_contact') 
                    if (two_contact_State_intersection == [] or ('V' in two_contact_State_intersection[0] and double_contact_contact_state[0][0:double_contact_contact_state[0].index('E')]!=double_contact_contact_state[1][0:double_contact_contact_state[1].index('E')])) and 'double_contact' in potential_contact_state_set[i] and 'single_contact' in potential_contact_state_set[j]:
                        if tuple(potential_contact_state_set[i]) in next_contact_state:
                            already_have = False
                            for k0 in range(len(next_contact_state[tuple(potential_contact_state_set[i])])):
                                if next_contact_state[tuple(potential_contact_state_set[i])][k0]==tuple(potential_contact_state_set[j]) and action_to_next_contact_state[tuple(potential_contact_state_set[i])][k0]=='Tip':
        ...
        return potential_contact_state_set, next_contact_state, action_to_next_contact_state, action_cost                    
        ...
        while OPEN_set != set():
            ...
            for contact_state, action_list in self.action_to_next_contact_state.items():
                if tuple(sorted(contact_state))==first_node:
                    ...
                    break
            for contact_state, next_contact_state_list in self.next_contact_state.items():
                if tuple(sorted(contact_state))==first_node:
