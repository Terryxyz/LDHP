...
#print from_real_position_to_index([[0.5+0.4,1], [1.4+0.4,1], [1.4+0.4,1.8], [1.3+0.4,1.9], [1.3+0.4,2.4], [0.95+0.4,2.45], [0.6+0.4,2.4], [0.6+0.4,1.9], [0.5+0.4,1.8]], [1.7,2])
...
#print from_index_to_real_position([[0.5+0.4,1], [1.4+0.4,1], [1.4+0.4,1.8], [1.3+0.4,1.9], [1.3+0.4,2.4], [0.95+0.4,2.45], [0.6+0.4,2.4], [0.6+0.4,1.9], [0.5+0.4,1.8]], 44)

def whether_finger_contact_pair_feasible(ini_vertex_position, ini_vertex_position_contract, F1_index, F2_index):
    if F1_index<=100 and F2_index<=100:
        ...
        if MultiPoint([F1_position, F2_position]).distance(MultiPoint(ini_vertex_position))<0.1:
            return False 
    elif F1_index>100 and F2_index>100:
        if (F1_index%100==F2_index%100 or F1_index%100==F2_index%100+36 or F2_index%100==F1_index%100+36)==False:
            return False
        F1_contact_position = ini_vertex_position[int(F1_index/100)-1]
        F1_angle = (F1_index%100-1)*5
        F1_contact_position2 = [F1_contact_position[0]+1*cos(F1_angle*pi/180), F1_contact_position[1]+1*sin(F1_angle*pi/180)]
        F2_contact_position = ini_vertex_position[int(F2_index/100)-1]
        F2_angle = (F2_index%100-1)*5
        F2_contact_position2 = [F2_contact_position[0]+1*cos(F2_angle*pi/180), F2_contact_position[1]+1*sin(F2_angle*pi/180)]
        parallel_distance = Point(F2_contact_position).distance(Point(footCtoAB(F1_contact_position,F1_contact_position2,F2_contact_position)))
        if sqrt(parallel_distance**2+2**2)<(Point(F1_contact_position).distance(Point(F2_contact_position)))**2:
            return False
        return True
    else:
        if F2_index <= 100:
            F2_position = from_index_to_real_position(ini_vertex_position, F2_index)
            if Point(F2_position).distance(MultiPoint(ini_vertex_position))<0.1:
                return False    
            F1_contact_position = ini_vertex_position[int(F1_index/100)-1]
            F1_angle = (F1_index%100-1)*5
            F1_contact_position2 = [F1_contact_position[0]+1*cos(F1_angle*pi/180), F1_contact_position[1]+1*sin(F1_angle*pi/180)]
            F1_position = footCtoAB(F1_contact_position,F1_contact_position2,F2_position)
        elif F1_index <= 100:
            F1_position = from_index_to_real_position(ini_vertex_position, F1_index)
            if Point(F1_position).distance(MultiPoint(ini_vertex_position))<0.1:
                return False    
            F2_contact_position = ini_vertex_position[int(F2_index/100)-1]
            F2_angle = (F2_index%100-1)*5
            F2_contact_position2 = [F2_contact_position[0]+1*cos(F2_angle*pi/180), F2_contact_position[1]+1*sin(F2_angle*pi/180)]
            F2_position = footCtoAB(F2_contact_position,F2_contact_position2,F1_position)
    F1F2distance=Point(F1_position).distance(Point(F2_position))
    if F1F2distance<0.05:
        return False
    if F1F2distance>1.4:
        return False
    ...
    F1_F1_F2_perp1 = [F1_position, [F1_position[0]+2*F1_F2_perp1[0], F1_position[1]+2*F1_F2_perp1[1]]]
    F2_F1_F2_perp1 = [F2_position, [F2_position[0]+2*F1_F2_perp1[0], F2_position[1]+2*F1_F2_perp1[1]]]
    F1_perp1_out = [[F1_position[0]+0.8*F1_F2_perp1[0], F1_position[1]+0.8*F1_F2_perp1[1]], [F1_position[0]+0.8*F1_F2_perp1[0]+0.35*F2_F1_vector[0], F1_position[1]+0.8*F1_F2_perp1[1]+0.35*F2_F1_vector[1]]]
    F2_perp1_out = [[F2_position[0]+0.8*F1_F2_perp1[0], F2_position[1]+0.8*F1_F2_perp1[1]], [F2_position[0]+0.8*F1_F2_perp1[0]+0.35*F1_F2_vector[0], F2_position[1]+0.8*F1_F2_perp1[1]+0.35*F1_F2_vector[1]]]
    F1_F1_F2_perp2 = [F1_position, [F1_position[0]+2*F1_F2_perp2[0], F1_position[1]+2*F1_F2_perp2[1]]]
    F2_F1_F2_perp2 = [F2_position, [F2_position[0]+2*F1_F2_perp2[0], F2_position[1]+2*F1_F2_perp2[1]]]
    F1_perp2_out = [[F1_position[0]+0.8*F1_F2_perp2[0], F1_position[1]+0.8*F1_F2_perp2[1]], [F1_position[0]+0.8*F1_F2_perp2[0]+0.35*F2_F1_vector[0], F1_position[1]+0.8*F1_F2_perp2[1]+0.35*F2_F1_vector[1]]]
    F2_perp2_out = [[F2_position[0]+0.8*F1_F2_perp2[0], F2_position[1]+0.8*F1_F2_perp2[1]], [F2_position[0]+0.8*F1_F2_perp2[0]+0.35*F1_F2_vector[0], F2_position[1]+0.8*F1_F2_perp2[1]+0.35*F1_F2_vector[1]]]
    if MultiLineString([F1_F1_F2_perp1, F2_F1_F2_perp1, F1_perp1_out, F2_perp1_out]).distance(ini_vertex_position_contract)>0 or MultiLineString([F1_F1_F2_perp2, F2_F1_F2_perp2, F1_perp2_out, F2_perp2_out]).distance(ini_vertex_position_contract)>0:
        return True
    else:
        return False

def feasible_finger_contact_pair_set(ini_vertex_position):
    F1_index_set = range(1,101)
    F2_index_set = range(1,101)
    grasp_index_list = [(i,j) for i in F1_index_set for j in F2_index_set]
    temp_list = grasp_index_list[:]
    ini_vertex_position_contract = Polygon(ini_vertex_position).buffer(-0.01)
    for grasp_index in temp_list:
        if grasp_index[0]==grasp_index[1]:
            grasp_index_list.remove(grasp_index)
        elif whether_finger_contact_pair_feasible(ini_vertex_position, ini_vertex_position_contract, grasp_index[0], grasp_index[1])==False:
            grasp_index_list.remove(grasp_index)
    return grasp_index_list

def feasible_fcp_avoid_env_collision(current_vertex_position, feasible_fcp_set, ini_vertex_position, profile_environment, accuracy = 0.002):
    avoid_index_set = []
    ...
    for vertex_index in range(len(current_vertex_position)):
        if Point(current_vertex_position[vertex_index]).distance(LineString(profile_environment))<accuracy:
            avoid_index_set += range(100*(vertex_index+1)+1, 100*(vertex_index+1)+73)
    ...
    feasible_fcp_set1 = list(xtuple(feasible_fcp_set1.tolist()))
    feasible_fcp_set2 = feasible_fcp_set1
    current_vertex_position_contract = Polygon(current_vertex_position).buffer(-0.005)
    feasible_gripper_pose_set = []
    for fcp in feasible_fcp_set2:
        gripper_pose_set = from_index_to_gripper_pose_set(current_vertex_position, current_vertex_position_contract, fcp[0], fcp[1], ini_vertex_position, profile_environment)
        if gripper_pose_set == []:
            feasible_fcp_set1.remove(fcp)
        feasible_gripper_pose_set+=gripper_pose_set
    return feasible_fcp_set1, feasible_gripper_pose_set
...
def obtain_feasible_fcp_set_of_each_pose(feasible_fcp_set, object_pose_trajectory, ini_vertex_position, profile_environment, fcp_set_index_memory = dict(), gripper_pose_set_index_memory=dict()):
    ...
    for i in range(len(object_pose_trajectory)):
        ...
        if xtuple(object_pose_discrete) in fcp_set_index_memory and xtuple(object_pose_discrete) in gripper_pose_set_index_memory:
            ...
            feasible_fcp_set1, feasible_gripper_pose_set1 = feasible_fcp_avoid_env_collision(object_pose_trajectory[i], fcp_index_list_temp, ini_vertex_position, profile_environment)
...
#time1=time.time()
#aaa=feasible_finger_contact_pair_set([[0.5,0], [1.2,0], [1.2,1.5], [0.5,1.5]])
#print len(aaa)
#print time.time()-time1
#kkk=feasible_fcp_avoid_env_collision([[0.5,0], [1.2,0], [1.2,1.5], [0.5,1.5]], aaa, [[0,0],[3,0]])
#print len(kkk)
#print set(kkk).issubset(set(aaa))
#print time.time()-time1           

def from_index_to_gripper_pose_set(current_vertex_position, current_vertex_position_contract, F1_index, F2_index, ini_vertex_position, profile_environment):
    if F1_index<=100 and F2_index<=100:
        ...
        F1_F2_vector = [(F2_position[0]-F1_position[0])/LineString([F1_position, F2_position]).length, (F2_position[1]-F1_position[1])/LineString([F1_position, F2_position]).length]
        F2_F1_vector = [-F1_F2_vector[0], -F1_F2_vector[1]]
        F1_F2_perp1 = [cos(angle*pi/180+pi/2), sin(angle*pi/180+pi/2)]
        F1_F2_perp2 = [cos(angle*pi/180-pi/2), sin(angle*pi/180-pi/2)]
        F1_F1_F2_perp1 = [F1_position, [F1_position[0]+2*F1_F2_perp1[0], F1_position[1]+2*F1_F2_perp1[1]]]
        F2_F1_F2_perp1 = [F2_position, [F2_position[0]+2*F1_F2_perp1[0], F2_position[1]+2*F1_F2_perp1[1]]]
        F1_perp1_out = [[F1_position[0]+0.8*F1_F2_perp1[0], F1_position[1]+0.8*F1_F2_perp1[1]], [F1_position[0]+0.8*F1_F2_perp1[0]+0.35*F2_F1_vector[0], F1_position[1]+0.8*F1_F2_perp1[1]+0.35*F2_F1_vector[1]]]
        F2_perp1_out = [[F2_position[0]+0.8*F1_F2_perp1[0], F2_position[1]+0.8*F1_F2_perp1[1]], [F2_position[0]+0.8*F1_F2_perp1[0]+0.35*F1_F2_vector[0], F2_position[1]+0.8*F1_F2_perp1[1]+0.35*F1_F2_vector[1]]]
        ...
        F2_perp2_out = [[F2_position[0]+0.8*F1_F2_perp2[0], F2_position[1]+0.8*F1_F2_perp2[1]], [F2_position[0]+0.8*F1_F2_perp2[0]+0.35*F1_F2_vector[0], F2_position[1]+0.8*F1_F2_perp2[1]+0.35*F1_F2_vector[1]]]
        gripper_pose_set = []
        if MultiLineString([F1_F1_F2_perp1, F2_F1_F2_perp1, F1_perp1_out, F2_perp1_out]).distance(LineString(profile_environment))>0.002 and MultiLineString([F1_F1_F2_perp1, F2_F1_F2_perp1, F1_perp1_out, F2_perp1_out]).distance(current_vertex_position_contract)>0:
            gripper_pose_set.append((center_position[0], center_position[1], aperture, angle+90, '+'))
        if MultiLineString([F1_F1_F2_perp2, F2_F1_F2_perp2, F1_perp2_out, F2_perp2_out]).distance(LineString(profile_environment))>0.002 and MultiLineString([F1_F1_F2_perp2, F2_F1_F2_perp2, F1_perp2_out, F2_perp2_out]).distance(current_vertex_position_contract)>0:
            gripper_pose_set.append((center_position[0], center_position[1], aperture, angle-90, '-'))
        return gripper_pose_set
#time1=time.time()
#print from_index_to_gripper_pose_set([[0.5,0], [1.2,0], [1.2,1.5], [0.5,1.5]], 419, 319, [[0,0], [0.7,0], [0.7,1.5], [0,1.5]], [[0,0], [3,0]])
#print time.time()-time1
...
for i in range(1,100):
    for j in range(1,100):
        if whether_fcp([[1.0, 0.4950000000000001], [1.495, 0.0], [2.556, 1.06], [2.06, 1.556]], i, j, [[1,0.7], [1,0], [2.5,0], [2.5,0.7]], [1.75,0.35] , 'Move-in-air', [[0, 1], 1.0], [[0,0.75], [0,0], [3,0], [3,0.75]])[0]==True:
            print i,j
'''
...
#feasible_fcp_set = feasible_finger_contact_pair_set([[1,0.7], [1,0], [2.5,0], [2.5,0.7]])
#object_pose_trajectory_plus_ini = [[[1,0.7], [1,0], [2.5,0], [2.5,0.7]], [[5.551115123125783e-17, 0.7], [5.551115123125783e-17, 0.0], [1.4999999999999998, 0.0], [1.4999999999999998, 0.7]], [[5.551115123125783e-17, 0.6062177826491071], [0.3499999999999999, 0.0], [1.6490381056766577, 0.7499999999999997], [1.2990381056766578, 1.3562177826491069]], [[0.20000000000000007, 0.6062177826491071], [0.5499999999999999, 0.0], [1.8490381056766576, 0.7499999999999997], [1.4990381056766577, 1.3562177826491069]], [[0.05502525316941681, 0.49497474683058335], [0.5499999999999999, 0.0], [1.6106601717798212, 1.0606601717798207], [1.115685424949238, 1.5556349186104044]], [[1.0, 0.4950000000000001], [1.495, 0.0], [2.556, 1.06], [2.06, 1.556]], [[1, 1.495], [1.495, 1], [2.556, 2.06], [2.06, 2.556]]]
#environment = [[0,0.75], [0,0], [3,0], [3,0.75]]
#initial_object_vertex_position = [[1,0.7], [1,0], [2.5,0], [2.5,0.7]]
#initial_CoM_position = [1.75, 0.35]
#feasible_fcp_set_list, _0 = obtain_feasible_fcp_set_of_each_pose(feasible_fcp_set, object_pose_trajectory_plus_ini, environment)
#action_sequence = ['Push', 'Tilting-slide', 'Push', 'Tip', 'Push', 'Move-in-air']
#action_parameter_sequence = [[[-1.0, 0.0], 1.0], [['V1E3', 'V2E2', 'double_contact'], 30], [[1.0, 0.0], 0.2], [[0.5499999999999999, 0.0], 'CCW', 14.999999999999996], [[1.0, 0.0], 0.9450000000000002], [[0, 1], 1.0]]
#suitable_fcp_list_sequence, _1 = obtain_suitable_fcp_set_of_each_action(action_sequence, action_parameter_sequence, object_pose_trajectory_plus_ini, feasible_fcp_set_list, initial_object_vertex_position, initial_CoM_position, environment)

#for i in range(len(feasible_fcp_set_list)):
    #print len(feasible_fcp_set_list[i])
#for j in range(len(suitable_fcp_list_sequence)):
    #print len(suitable_fcp_list_sequence[j])
...
def gripper_pose_map_moveobject(action_sequence, action_parameter_sequence, object_pose_trajectory_plus_ini, suitable_fcp_list_sequence, profile_environment):
    ...
    for i in range(len(action_sequence)):
        object_pose_trajectory_plus_ini_i_contract = Polygon(object_pose_trajectory_plus_ini[i]).buffer(-0.005)
        for j in range(len(suitable_fcp_list_sequence[i])):
            gripper_pose_set_begin = from_index_to_gripper_pose_set(object_pose_trajectory_plus_ini[i], object_pose_trajectory_plus_ini_i_contract, suitable_fcp_list_sequence[i][j][0], suitable_fcp_list_sequence[i][j][1], object_pose_trajectory_plus_ini[0], profile_environment)
...
def gripper_pose_map_adjustgrasp(object_pose_trajectory_plus_ini, feasible_fcp_set_list, feasible_gripper_pose_set_list, gripper_pose_map_moveobject_link_set, ini_gripper_pose, profile_environment, ini_vertex_position, ini_CoM_position):
    ...
    for i in range(len(object_pose_trajectory_plus_ini)):
        ...
        if ini_gripper_pose != None:
            ini_F1, ini_F2, ini_gripper_profile =gripper_profile(ini_gripper_pose)  
            ini_F1_link = ini_gripper_profile.geoms[0]
            ini_F2_link = ini_gripper_profile.geoms[1]
            contact_index, contact_list, contact_normal_list, contact_local_tangent_list = contact_points_and_normal_consider_gripper_type2_finger(object_pose_trajectory_plus_ini[0], ini_F1, ini_F2, ini_F1_link, ini_F2_link)
        if i==0 and ini_gripper_pose != None and whether_AdjustGraspAction_feasible(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, 'MoveGripper')==True:   # need change for gripper 2
            ...
            for j in range(len(moveobj_link_end_equal_i_gripper_pose)):
                ...
                if i>=1 and moveobj_link_end_equal_i_gripper_pose[j] not in gripper_pose_reachable_current_i:
                    continue
                for feasible_gripper_pose in moveobj_link_start_equal_i_gripper_pose:
                    if abs(moveobj_link_end_equal_i_gripper_pose[j][0]-feasible_gripper_pose[0])<0.015 and abs(moveobj_link_end_equal_i_gripper_pose[j][1]-feasible_gripper_pose[1])<0.015 and abs(moveobj_link_end_equal_i_gripper_pose[j][2]-feasible_gripper_pose[2])<0.01 and abs(moveobj_link_end_equal_i_gripper_pose[j][3]-feasible_gripper_pose[3])<1 and moveobj_link_end_equal_i_gripper_pose[j][4]==feasible_gripper_pose[4]:
                ...
                current_F1, current_F2, current_gripper_profile =gripper_profile(moveobj_link_end_equal_i_gripper_pose[j])
                current_F1_link = current_gripper_profile.geoms[0]
                current_F2_link = current_gripper_profile.geoms[1]                                                                
                temp_contact_index_finger, temp_contact_list_finger, temp_contact_normal_list_finger, temp_contact_local_tangent_list_finger = contact_points_and_normal_consider_gripper_type2_finger(object_pose_trajectory_plus_ini[i], current_F1, current_F2, current_F1_link, current_F2_link) # need change for gripper 2
                ...
                feasible_action_list = AdjustGraspAction_feasible_list(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM)  # need change for gripper 2
                    ...
                    if action == 'MoveGripper':
                        ...
                        for item in target_gripper_pose_set:
                            ...
                            for feasible_gripper_pose in feasible_gripper_pose_set_list[i]:
                                if abs(item[0][0]-feasible_gripper_pose[0])<0.015 and abs(item[0][1]-feasible_gripper_pose[1])<0.015 and abs(item[0][2]-feasible_gripper_pose[2])<0.01 and abs(item[0][3]-feasible_gripper_pose[3])<1 and item[0][4]==feasible_gripper_pose[4]:
            ...
            for j in range(len(feasible_gripper_pose_subset_i)):
                ...
                current_F1, current_F2, current_gripper_profile =gripper_profile(feasible_gripper_pose_subset_i[j])
                current_F1_link = current_gripper_profile.geoms[0]
                current_F2_link = current_gripper_profile.geoms[1]        
                temp_contact_index_finger, temp_contact_list_finger, temp_contact_normal_list_finger, temp_contact_local_tangent_list_finger = contact_points_and_normal_consider_gripper_type2_finger(object_pose_trajectory_plus_ini[i], current_F1, current_F2, current_F1_link, current_F2_link) # need change for gripper 2
                contact_index = temp_contact_index+temp_contact_index_finger
    ...
    if achievable_index != len(object_pose_trajectory_plus_ini)-2:
        print(achievable_index, len(object_pose_trajectory_plus_ini)-2)
        #m=k+l
        return []
    return gripper_pose_map_adjustgrasp_link_set


def node_eaual(node1, node2):
    if node1[0]==node2[0]:
        if abs(node1[1][0]-node2[1][0])<0.01 and abs(node1[1][1]-node2[1][1])<0.01 and abs(node1[1][2]-node2[1][2])<0.01 and abs(node1[1][3]-node2[1][3])<1:
            return True
    else:
        return False
...
def A_star_find_action_path(ini_gripper_pose, gripper_pose_map_moveobject_link_set, gripper_pose_map_adjustgrasp_link_set, object_pose_trajectory_plus_ini, feasible_fcp_set_list, initial_CoM_position, profile_environment):
    ...
    if whether_AdjustGraspAction_feasible(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, initial_CoM_position, 'MoveGripper', ini_F1, ini_F2)==False:
        object_pose_trajectory_plus_ini_0_contract = Polygon(object_pose_trajectory_plus_ini[0]).buffer(-0.005)
        for j in range(len(feasible_fcp_set_list[0])):
            ini_feasible_gripper_pose_subset+=from_index_to_gripper_pose_set(object_pose_trajectory_plus_ini[0], object_pose_trajectory_plus_ini_0_contract, feasible_fcp_set_list[0][j][0], feasible_fcp_set_list[0][j][1], object_pose_trajectory_plus_ini[0], profile_environment)
        for discrete_gripper_pose in ini_feasible_gripper_pose_subset:
            if abs(ini_gripper_pose[0]-discrete_gripper_pose[0])<0.015 and abs(ini_gripper_pose[1]-discrete_gripper_pose[1])<0.015 and abs(ini_gripper_pose[2]-discrete_gripper_pose[2])<0.01 and abs(ini_gripper_pose[3]-discrete_gripper_pose[3])<1:
    ...
    while OPEN != set():
        ...
        for link in map_link_set:
            ...
            if node_eaual(tuple(link[0]), first_node)==True and tuple(link[1]) not in CLOSED:
                print('link', link)
...
def AdjustGraspAction_feasible_list(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, F1_contact_point=None, F2_contact_point=None):
    feasible_action_list = []
    for action in ['MoveGripper','PivotF1CW','PivotF1CCW','PivotF2CW','PivotF2CCW', 'Pivot+Flip+Pivot_F1CW','Pivot+Flip+Pivot_F1CCW','Pivot+Flip+Pivot_F2CW','Pivot+Flip+Pivot_F2CCW','Slide+PivotF1CW', 'Slide+PivotF1CCW', 'Slide-PivotF1CW', 'Slide-PivotF1CCW','Slide+PivotF2CW', 'Slide+PivotF2CCW', 'Slide-PivotF2CW', 'Slide-PivotF2CCW', 'Slide+PivotF1CWClose', 'Slide+PivotF1CCWClose', 'Slide-PivotF1CWClose', 'Slide-PivotF1CCWClose','Slide+PivotF2CWClose', 'Slide+PivotF2CCWClose', 'Slide-PivotF2CWClose', 'Slide-PivotF2CCWClose']:
        if whether_AdjustGraspAction_feasible(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, action, F1_contact_point, F2_contact_point)==True:
            feasible_action_list.append(action)
    return feasible_action_list
#done

# only consider stability
def whether_AdjustGraspAction_feasible(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, action, F1_contact_point=None, F2_contact_point=None):
    temp_contact_index = contact_index[:]
    if F2_contact_point != None and 'F1' in temp_contact_index and Point(contact_list[temp_contact_index.index('F1')]).distance(Point(F2_contact_point))<0.01:
        contact_index[temp_contact_index.index('F1')]='F2'
    if F1_contact_point != None and 'F2' in temp_contact_index and Point(contact_list[temp_contact_index.index('F2')]).distance(Point(F1_contact_point))<0.01:
    ...
    if contact_finger_number==0:
        ...
        if 'F1' in contact_index:
            if action == 'MoveGripper':
                ...
                if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode0)[0]==True:
                    return True
            elif action == 'PivotF1CW' or action == 'PivotF1CCW' or action == 'Pivot+Flip+Pivot_F1CCW' or action == 'Pivot+Flip+Pivot_F1CW':
                contact_mode1 = dict({'F1':'R'}, **dict.fromkeys(contact_index_E, 'R'))
                if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode1)[0]==True:
                    return True
                ...
                if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode2)[0]==True:
                    return True
            elif action == 'Slide-PivotF1CW' or action == 'Slide-PivotF1CCW' or action == 'Slide-PivotF1CWClose' or action == 'Slide-PivotF1CCWClose':
                contact_mode3 = dict({'F1':'S-'}, **dict.fromkeys(contact_index_E, 'R'))
                if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode3)[0]==True:
                    return True
            ...
            if action == 'MoveGripper':
                ...
                if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode0)[0]==True:
                    return True
            elif action == 'PivotF2CW' or action == 'PivotF2CCW' or action == 'Pivot+Flip+Pivot_F2CCW' or action == 'Pivot+Flip+Pivot_F2CW':
        ...
        if action == 'MoveGripper':
            ...
            if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode0)[0]==True:
                return True
        if action == 'PivotF1CW' or action == 'PivotF1CCW' or action == 'Pivot+Flip+Pivot_F1CCW' or action == 'Pivot+Flip+Pivot_F1CW':
            contact_mode1 = dict({'F1':'R', 'F2': 'B'}, **dict.fromkeys(contact_index_E, 'R'))
            if whether_stable(contact_index, contact_list, contact_normal_list, contact_local_tangent_list, current_CoM, contact_mode1)[0]==True:
                return True
        if action == 'PivotF2CW' or action == 'PivotF2CCW' or action == 'Pivot+Flip+Pivot_F2CCW' or action == 'Pivot+Flip+Pivot_F2CW':
...
def finger_position_after_action(current_vertex_position, current_gripper_pose, action, profile_environment, ini_vertex_position, ini_CoM_position, accuracy=0.01):
    current_F1, current_F2, gripper_profile0 = gripper_profile(current_gripper_pose)
    current_vertex_position_contract = Polygon(current_vertex_position).buffer(-0.005)
    ...
    if action == 'PivotF1CCW' or action == 'PivotF1CW' or action == 'PivotF2CCW' or action == 'PivotF2CW':
        if 'F1' in action:
            pole = current_F1
            if Point(pole).distance(Polygon(current_vertex_position))>accuracy:
                return []
        else:
            pole = current_F2
            if Point(pole).distance(Polygon(current_vertex_position))>accuracy:
        ...
        if 'F1CCW' in action:
            ...
            effector_trajectory = MultiLineString(list(chain(*[rotate(gripper_profile0, angle, origin=Point(current_F1), use_radians=True).geoms for angle in np.arange(1*pi/180, target_rotation_angle-0.2*pi/180, 1*pi/180)])))
        elif 'F1CW' in action:
            target_rotation_angle_set = [(inclination_angle(current_F1,point)-current_angle)%(-2*pi) for point in intersection_circle_object]
            ...
            effector_trajectory = MultiLineString(list(chain(*[rotate(gripper_profile0, angle, origin=Point(current_F2), use_radians=True).geoms for angle in np.arange(1*pi/180, target_rotation_angle-0.2*pi/180, 1*pi/180)])))
        elif 'F2CW' in action:
            target_rotation_angle_set = [(inclination_angle(current_F2,point)-current_angle)%(-2*pi) for point in intersection_circle_object]
            target_rotation_angle = max(target_rotation_angle_set)
            target_F1 = intersection_circle_object[target_rotation_angle_set.index(target_rotation_angle)]
            effector_trajectory = MultiLineString(list(chain(*[rotate(gripper_profile0, angle, origin=Point(current_F2), use_radians=True).geoms for angle in np.arange(-1*pi/180, target_rotation_angle+0.2*pi/180, -1*pi/180)])))
        if effector_trajectory.distance(Polygon(current_vertex_position).buffer(-0.002))==0 or effector_trajectory.distance(LineString(profile_environment))<0.01:
            return []
        ...
        angle = current_gripper_pose[3]+target_rotation_angle*180/pi
        target_gripper_pose_set.append([(center_position[0], center_position[1], aperture, angle, current_gripper_pose[4]), cost, [['rotate', pole, rotate_direction, abs(target_rotation_angle*180/pi)]]]) 
        ...
        if 'F1' in action:
            if Point(current_F1).distance(Polygon(current_vertex_position))>accuracy:
                return []
        else:
            if Point(current_F2).distance(Polygon(current_vertex_position))>accuracy:
                return []        
        ...
        for sliding_length in np.arange(0.05, max_sliding_distance, 0.05):
            current_gripper_pose_new = [current_gripper_pose[0]+sliding_length*slide_direction[0], current_gripper_pose[1]+sliding_length*slide_direction[1], current_gripper_pose[2], current_gripper_pose[3], current_gripper_pose[4]]
            current_F1_new, current_F2_new, gripper_profile_new = gripper_profile(current_gripper_pose_new)
            if 'F1' in action:
                pole = current_F1_new
            else:
                pole = current_F2_new
            if 'CCW' in action:
                ...
                rotate_direction = 'CW'            
            if gripper_profile_new.distance(current_vertex_position_contract)==0 or gripper_profile_new.distance(LineString(profile_environment))<0.01 or MultiPoint([current_F1_new, current_F2_new]).distance(MultiPoint(current_vertex_position))<0.01:
                break
            if 'F1CCW' in action:
            ...
            if 'F1CCW' in action:
                ...
                target_rotation_angle = min(target_rotation_angle_set)
                target_F2 = intersection_circle_object[target_rotation_angle_set.index(target_rotation_angle)]
                effector_trajectory = MultiLineString(list(chain(*[rotate(gripper_profile_new, angle, origin=Point(current_F1_new), use_radians=True).geoms for angle in np.arange(1*pi/180, target_rotation_angle-0.2*pi/180, 1*pi/180)])))
            elif 'F1CW' in action:
                target_rotation_angle_set = [(inclination_angle(current_F1_new,point)-current_angle)%(-2*pi) for point in intersection_circle_object]
                target_rotation_angle = max(target_rotation_angle_set)
                target_F2 = intersection_circle_object[target_rotation_angle_set.index(target_rotation_angle)]
                ...
                target_rotation_angle = min(target_rotation_angle_set)
                target_F1 = intersection_circle_object[target_rotation_angle_set.index(target_rotation_angle)]
                effector_trajectory = MultiLineString(list(chain(*[rotate(gripper_profile_new, angle, origin=Point(current_F2_new), use_radians=True).geoms for angle in np.arange(1*pi/180, target_rotation_angle-0.2*pi/180, 1*pi/180)])))
            elif 'F2CW' in action:
                target_rotation_angle_set = [(inclination_angle(current_F2_new,point)-current_angle)%(-2*pi) for point in intersection_circle_object]
                target_rotation_angle = max(target_rotation_angle_set)
                target_F1 = intersection_circle_object[target_rotation_angle_set.index(target_rotation_angle)]
            ...
            center_position = [(target_F1[0]+target_F2[0])/2, (target_F1[1]+target_F2[1])/2]
            angle = current_gripper_pose[3]+target_rotation_angle*180/pi
            target_gripper_pose_set.append([(center_position[0], center_position[1], aperture, angle, current_gripper_pose[4]), cost, [['slide', slide_direction, sliding_length], ['rotate', pole, rotate_direction, abs(target_rotation_angle*180/pi)]]]) 
    elif action in ['Slide+PivotF1CWClose', 'Slide+PivotF1CCWClose', 'Slide-PivotF1CWClose', 'Slide-PivotF1CCWClose', 'Slide+PivotF2CWClose', 'Slide+PivotF2CCWClose', 'Slide-PivotF2CWClose', 'Slide-PivotF2CCWClose']:
        if 'F1' in action:
            if Point(current_F1).distance(Polygon(current_vertex_position))>accuracy:
                return []
        else:
        ...
        for sliding_length in np.arange(0.05, max_sliding_distance, 0.05):
            ...
            if gripper_profile_new.distance(current_vertex_position_contract)==0 or gripper_profile_new.distance(LineString(profile_environment))<0.01 or MultiPoint([current_F1_new, current_F2_new]).distance(MultiPoint(current_vertex_position))<0.01:
                break
            accumulate_angle=0
            while accumulate_angle<360:
                accumulate_angle+=1
                if 'F1CCW' in action:
                    gripper_profile_new = rotate(gripper_profile_new, 1, origin=Point(current_F1_new))
                    if gripper_profile_new.distance(current_vertex_position_contract)==0 or gripper_profile_new.distance(LineString(profile_environment))<0.01:
                        break
                elif 'F1CW' in action:
                    gripper_profile_new = rotate(gripper_profile_new, -1, origin=Point(current_F1_new))
                    if gripper_profile_new.distance(current_vertex_position_contract)==0 or gripper_profile_new.distance(LineString(profile_environment))<0.01:
                    ...
                    gripper_profile_new = rotate(gripper_profile_new, 1, origin=Point(current_F2_new))
                    if gripper_profile_new.distance(current_vertex_position_contract)==0 or gripper_profile_new.distance(LineString(profile_environment))<0.01:
                        break
                elif 'F2CW' in action:
                    gripper_profile_new = rotate(gripper_profile_new, -1, origin=Point(current_F2_new))
                    if gripper_profile_new.distance(current_vertex_position_contract)==0 or gripper_profile_new.distance(LineString(profile_environment))<0.01:
                        break
                if 'F1' in action:
                    F2_to_F1_vector = (np.array(gripper_profile_new.geoms[0].coords[:][0])-np.array(gripper_profile_new.geoms[1].coords[:][0])).tolist()
                    F2_to_F1_vector_length = sqrt(F2_to_F1_vector[0]**2+F2_to_F1_vector[1]**2)
                    F2_to_F1_vector = [F2_to_F1_vector[0]/F2_to_F1_vector_length, F2_to_F1_vector[1]/F2_to_F1_vector_length]
                    gripper_profile_new0 = gripper_profile_new
                    gripper_profile_new1 = gripper_profile_new
                    max_closing_distance = F2_to_F1_vector_length
                    min_closing_distance = 0
                    i0 = 0
                    while i0 < 6:
                        i0+=1
                        gripper_profile_new1_geoms1 = translate(gripper_profile_new0.geoms[1], xoff=max_closing_distance*F2_to_F1_vector[0], yoff=max_closing_distance*F2_to_F1_vector[1])
                        gripper_profile_new1_geoms3 = translate(gripper_profile_new0.geoms[3], xoff=max_closing_distance*F2_to_F1_vector[0], yoff=max_closing_distance*F2_to_F1_vector[1])
                        gripper_profile_new1 = MultiLineString([gripper_profile_new1.geoms[0], gripper_profile_new1_geoms1, gripper_profile_new1.geoms[2], gripper_profile_new1_geoms3])
                        if gripper_profile_new1_geoms1.distance(Polygon(current_vertex_position))<0.005:
                            delta_distance = max_closing_distance-min_closing_distance
                            max_closing_distance = min_closing_distance+delta_distance/2
                        else:
                            delta_distance = max_closing_distance-min_closing_distance
                            min_closing_distance = max_closing_distance
                            max_closing_distance = max_closing_distance+delta_distance/2
                    if gripper_profile_new1.distance(LineString(profile_environment))>0.01 and Polygon(gripper_profile_new0.geoms[1].coords[:]+gripper_profile_new1_geoms1.coords[:]).distance(current_vertex_position_contract)>0:
                        cost = sliding_length+LineString([current_F1, current_F2]).length*accumulate_angle*pi/180+max_closing_distance+3
                        #print [k.coords[:] for k in gripper_profile_new1.geoms], gripper_pose_from_profile(gripper_profile_new1)
                        if 'CCW' in action:
                            rotate_direction = 'CCW'
                        else:
                            rotate_direction = 'CW'
                        target_gripper_pose_set.append([gripper_pose_from_profile(gripper_profile_new1), cost, [['slide', slide_direction, sliding_length], ['rotate', current_F1_new, rotate_direction, abs(accumulate_angle)], ['close', 'F2', F2_to_F1_vector, max_closing_distance]]])

                else:
                    F1_to_F2_vector = (np.array(gripper_profile_new.geoms[1].coords[:][0])-np.array(gripper_profile_new.geoms[0].coords[:][0])).tolist()
                    F1_to_F2_vector_length = sqrt(F1_to_F2_vector[0]**2+F1_to_F2_vector[1]**2)
                    F1_to_F2_vector = [F1_to_F2_vector[0]/F1_to_F2_vector_length, F1_to_F2_vector[1]/F1_to_F2_vector_length]
                    gripper_profile_new0 = gripper_profile_new
                    gripper_profile_new1 = gripper_profile_new
                    max_closing_distance = F1_to_F2_vector_length
                    min_closing_distance = 0
                    i0 = 0
                    while i0 < 6:
                        i0+=1
                        gripper_profile_new1_geoms0 = translate(gripper_profile_new0.geoms[0], xoff=max_closing_distance*F1_to_F2_vector[0], yoff=max_closing_distance*F1_to_F2_vector[1])
                        gripper_profile_new1_geoms2 = translate(gripper_profile_new0.geoms[2], xoff=max_closing_distance*F1_to_F2_vector[0], yoff=max_closing_distance*F1_to_F2_vector[1])
                        gripper_profile_new1 = MultiLineString([gripper_profile_new1_geoms0, gripper_profile_new1.geoms[1], gripper_profile_new1_geoms2, gripper_profile_new1.geoms[3]])
                        if gripper_profile_new1_geoms0.distance(Polygon(current_vertex_position))<0.005:
                            delta_distance = max_closing_distance-min_closing_distance
                            max_closing_distance = min_closing_distance+delta_distance/2
                        else:
                            delta_distance = max_closing_distance-min_closing_distance
                            min_closing_distance = max_closing_distance
                            max_closing_distance = max_closing_distance+delta_distance/2
                    if gripper_profile_new1.distance(LineString(profile_environment))>0.01 and Polygon(gripper_profile_new0.geoms[0].coords[:]+gripper_profile_new1_geoms0.coords[:]).distance(current_vertex_position_contract)>0:
                        cost = sliding_length+LineString([current_F1, current_F2]).length*accumulate_angle*pi/180+max_closing_distance+3
                        if 'CCW' in action:
                            rotate_direction = 'CCW'
                        else:
                            rotate_direction = 'CW'
                        target_gripper_pose_set.append([gripper_pose_from_profile(gripper_profile_new1), cost, [['slide', slide_direction, sliding_length], ['rotate', current_F2_new, rotate_direction, abs(accumulate_angle)], ['close', 'F1', F1_to_F2_vector, max_closing_distance]]])
    elif action in ['Pivot+Flip+Pivot_F1CCW', 'Pivot+Flip+Pivot_F1CW', 'Pivot+Flip+Pivot_F2CCW', 'Pivot+Flip+Pivot_F2CW']:
        if 'F1' in action:
            if Point(current_F1).distance(Polygon(current_vertex_position))>accuracy:
                return []
        else:
        ...
        if ('+' in action and 'F1' in action) or ('-' in action and 'F1' in action):
            ...
            for i in range(len(current_vertex_position)):
                if Point(current_F2).distance(LineString([current_vertex_position[i],current_vertex_position[(i+1)%(len(current_vertex_position))]]))<min_distance:
                    ...
                    min_distance = Point(current_F2).distance(LineString([current_vertex_position[i],current_vertex_position[(i+1)%(len(current_vertex_position))]]))
            current_F2 = footCtoAB(contact_edge[0],contact_edge[1],current_F2)
        gripper_profile_new = gripper_profile0
        accumulate_angle = 0
        target_gripper_pose_set = []
        while abs(calculate_intersect_angle_AB_CD(contact_edge[0], contact_edge[1], gripper_profile_new.geoms[0].coords[:][0], gripper_profile_new.geoms[1].coords[:][0]))>1.5 and abs(calculate_intersect_angle_AB_CD(contact_edge[0], contact_edge[1], gripper_profile_new.geoms[0].coords[:][0], gripper_profile_new.geoms[1].coords[:][0])-180)>1.5 and abs(calculate_intersect_angle_AB_CD(contact_edge[0], contact_edge[1], gripper_profile_new.geoms[0].coords[:][0], gripper_profile_new.geoms[1].coords[:][0])+180)>1.5:
            accumulate_angle+=1
            if 'F1CCW' in action:
                gripper_profile_new = rotate(gripper_profile_new, 1, origin=Point(current_F1))
                if gripper_profile_new.distance(current_vertex_position_contract)==0 or gripper_profile_new.distance(LineString(profile_environment))<0.01:
                    break
            elif 'F1CW' in action:
                gripper_profile_new = rotate(gripper_profile_new, -1, origin=Point(current_F1))
                ...
                gripper_profile_new = rotate(gripper_profile_new, 1, origin=Point(current_F2))
                if gripper_profile_new.distance(current_vertex_position_contract)==0 or gripper_profile_new.distance(LineString(profile_environment))<0.01:
                    break
            elif 'F2CW' in action:
                gripper_profile_new = rotate(gripper_profile_new, -1, origin=Point(current_F2))
            ...
            cost0 = LineString([current_F1, current_F2]).length*accumulate_angle*pi/180
            accumulate_angle0 = accumulate_angle
            if 'F1' in action:
                axis_A=current_F1
                axis_B=[current_F1[0]+generate_vector(contact_edge[0],contact_edge[1])[1]*1, current_F1[1]-generate_vector(contact_edge[0],contact_edge[1])[0]*1]
            else:
                axis_A=current_F2
                axis_B=[current_F2[0]+generate_vector(contact_edge[0],contact_edge[1])[1]*1, current_F2[1]-generate_vector(contact_edge[0],contact_edge[1])[0]*1]
            a, b, c, d, e, f = calculate_axial_symmetric_point_matrix(axis_A, axis_B)
            gripper_profile_new = affine_transform(gripper_profile_new, [a, b, d, e, c, f])
            accumulate_angle=0
            while True:
                accumulate_angle+=1
                if 'F1CCW' in action:
                    ...
                    gripper_profile_new = rotate(gripper_profile_new, -1, origin=Point(current_F2))
                if gripper_profile_new.distance(LineString(profile_environment))<0.01:
                    break
                if 'F1' in action:
                    if Point(gripper_profile_new.geoms[1].coords[:][0]).distance(Polygon(current_vertex_position))<0.005:
                        cost = cost0 + LineString([current_F1, current_F2]).length*accumulate_angle*pi/180+3
                        if 'CCW' in action:
                            rotate_direction = 'CCW'
                        else:
                            rotate_direction = 'CW'
                        target_gripper_pose_set.append([gripper_pose_from_profile(gripper_profile_new),cost, [['rotate', current_F1, rotate_direction, abs(accumulate_angle0)], ['flip', axis_A, axis_B], ['rotate', current_F1, rotate_direction, abs(accumulate_angle)]]]) 
                        break
                else:
                    if Point(gripper_profile_new.geoms[0].coords[:][0]).distance(Polygon(current_vertex_position))<0.005:
                        cost = cost0 + LineString([current_F1, current_F2]).length*accumulate_angle*pi/180+3
                        if 'CCW' in action:
                            rotate_direction = 'CCW'
                        else:
                            rotate_direction = 'CW'
                        target_gripper_pose_set.append([gripper_pose_from_profile(gripper_profile_new),cost, [['rotate', current_F2, rotate_direction, abs(accumulate_angle0)], ['flip', axis_A, axis_B], ['rotate', current_F2, rotate_direction, abs(accumulate_angle)]]]) 
                        break
    return target_gripper_pose_set  
#done

def gripper_profile(current_gripper_pose):
    if current_gripper_pose[4]=='+':
        ...
        current_F1 = [current_gripper_pose[0]+current_gripper_pose[2]/2.0*cos((current_gripper_pose[3]-90)*pi/180), current_gripper_pose[1]+current_gripper_pose[2]/2.0*sin((current_gripper_pose[3]-90)*pi/180)]
        current_F2 = [current_gripper_pose[0]-current_gripper_pose[2]/2.0*cos((current_gripper_pose[3]-90)*pi/180), current_gripper_pose[1]-current_gripper_pose[2]/2.0*sin((current_gripper_pose[3]-90)*pi/180)]        
    aperture = current_gripper_pose[2]
    F1_F2_vector = [(current_F2[0]-current_F1[0])/LineString([current_F1, current_F2]).length, (current_F2[1]-current_F1[1])/LineString([current_F1, current_F2]).length]
    F2_F1_vector = [-F1_F2_vector[0], -F1_F2_vector[1]]
    F1_F2_perp1 = [cos(current_gripper_pose[3]*pi/180), sin(current_gripper_pose[3]*pi/180)]
    F1_F1_F2_perp1 = [current_F1, [current_F1[0]+2*F1_F2_perp1[0], current_F1[1]+2*F1_F2_perp1[1]]]
    F2_F1_F2_perp1 = [current_F2, [current_F2[0]+2*F1_F2_perp1[0], current_F2[1]+2*F1_F2_perp1[1]]]
    F1_perp1_out = [[current_F1[0]+0.8*F1_F2_perp1[0], current_F1[1]+0.8*F1_F2_perp1[1]], [current_F1[0]+0.8*F1_F2_perp1[0]+0.35*F2_F1_vector[0], current_F1[1]+0.8*F1_F2_perp1[1]+0.35*F2_F1_vector[1]]]
    F2_perp1_out = [[current_F2[0]+0.8*F1_F2_perp1[0], current_F2[1]+0.8*F1_F2_perp1[1]], [current_F2[0]+0.8*F1_F2_perp1[0]+0.35*F1_F2_vector[0], current_F2[1]+0.8*F1_F2_perp1[1]+0.35*F1_F2_vector[1]]]
    return current_F1, current_F2, MultiLineString([F1_F1_F2_perp1, F2_F1_F2_perp1, F1_perp1_out, F2_perp1_out])
#done

#print gripper_profile(([1,1,1,90,'-']))

def gripper_pose_from_profile(gripper_profile):
    current_F1 = gripper_profile.geoms[0].coords[:][0]
    current_F2 = gripper_profile.geoms[1].coords[:][0]
    center_position = [(current_F1[0]+current_F2[0])/2, (current_F1[1]+current_F2[1])/2]
    aperture = Point(current_F1).distance(Point(current_F2))
    angle = inclination_angle(gripper_profile.geoms[0].coords[:][0],gripper_profile.geoms[0].coords[:][1])*180/pi
    if np.cross(generate_vector(current_F1, current_F2), [cos(angle*pi/180), sin(angle*pi/180)])<0:
        return (center_position[0], center_position[1], aperture, angle, '-')
    else:
        return (center_position[0], center_position[1], aperture, angle, '+')
#done
#print object_trajectory_moveobject([[5.551115123125783e-17, 0.7], [5.551115123125783e-17, 0.0], [1.4999999999999998, 0.0], [1.4999999999999998, 0.7]], 'Tilting-slide', [['V1E3', 'V2E2', 'double_contact'], 60], [[0,0.75], [0,0], [3,0], [3,0.75]])
#print gripper_trajectory_moveobject([[5.551115123125783e-17, 0.7], [5.551115123125783e-17, 0.0], [1.4999999999999998, 0.0], [1.4999999999999998, 0.7]], (1.1569999999999996, 0.5810000000000001, 0.7261129388738369, 70.86635679409454, '+'), 'Tilting-slide', [['V1E3', 'V2E2', 'double_contact'], 60], [[0,0.75], [0,0], [3,0], [3,0.75]])

#print gripper_pose_from_profile(gripper_profile((0,0,0.5,90,'-'))[2])
...
def action_trajectory(current_gripper_pose, parameter_set):
    ...
    for parameter in parameter_set:
        if parameter[0]=='rotate':
            ...
            a, b, c, d, e, f = calculate_axial_symmetric_point_matrix(parameter[1], parameter[2])
            current_gripper_profile = affine_transform(current_gripper_profile, [a, b, d, e, c, f])
            trajectory.append(gripper_pose_from_profile(current_gripper_profile))
        elif parameter[0]=='close':
            if parameter[1]=='F1':
                for distance in np.hstack((np.arange(0, parameter[3], 0.1),parameter[3])): 
                    new_gripper_profile_0 = translate(current_gripper_profile.geoms[0], xoff=distance*parameter[2][0], yoff=distance*parameter[2][1])
                    new_gripper_profile_2 = translate(current_gripper_profile.geoms[2], xoff=distance*parameter[2][0], yoff=distance*parameter[2][1])
                    new_gripper_profile = MultiLineString([new_gripper_profile_0, current_gripper_profile.geoms[1], new_gripper_profile_2, current_gripper_profile.geoms[3]])
                    trajectory.append(gripper_pose_from_profile(new_gripper_profile))
            else:
                for distance in np.hstack((np.arange(0, parameter[3], 0.1),parameter[3])): 
                    new_gripper_profile_1 = translate(current_gripper_profile.geoms[1], xoff=distance*parameter[2][0], yoff=distance*parameter[2][1])
                    new_gripper_profile_3 = translate(current_gripper_profile.geoms[3], xoff=distance*parameter[2][0], yoff=distance*parameter[2][1])
                    new_gripper_profile = MultiLineString([current_gripper_profile.geoms[0], new_gripper_profile_1, current_gripper_profile.geoms[2], new_gripper_profile_3])
                    trajectory.append(gripper_pose_from_profile(new_gripper_profile))
            current_gripper_profile = new_gripper_profile
    return trajectory
#done
#print action_trajectory((0.5, 0.5, 0.6, 90), [['rotate', [0.2,0.5], 'CCW', 90], ['slide', [0,1], 0.5], ['flip', [0.7,0], [0.7,2]], ['close', 'F1', [0,-1], 0.3]])


# ['Push', 'Tilting-slide', 'Push', 'Tip', 'Push', 'Move-in-air'] [['double_contact', 'l1E3', 'l2E2'], ['V1E3', 'V2E2', 'double_contact'], ['V2E2', 'single_contact'], ['V2E2', 'single_contact'], ['V2E2', 'single_contact'], ['no_contact']] [[[-1.0, 0.0], 1.0], [['V1E3', 'V2E2', 'double_contact'], 60], [[1.0, 0.0], 0.2], [[0.8062177826491068, 0.0], 'CW', 14.999999999999996], [[1.0, 0.0], 0.6887822173508933], [[0, 1], 1.0]] [[[5.551115123125783e-17, 0.7], [5.551115123125783e-17, 0.0], [1.4999999999999998, 0.0], [1.4999999999999998, 0.7]], [[0.0, 0.3500000000000002], [0.6062177826491069, 0.0], [1.3562177826491073, 1.2990381056766576], [0.7500000000000004, 1.6490381056766577]], [[0.2, 0.3500000000000002], [0.8062177826491068, 0.0], [1.5562177826491073, 1.2990381056766576], [0.9500000000000004, 1.6490381056766577]], [[0.31124303581852375, 0.4949747468305834], [0.8062177826491068, 0.0], [1.8668779544289285, 1.060660171779821], [1.3719032075983453, 1.5556349186104041]], [[1.0, 0.4950000000000001], [1.495, 0.0], [2.556, 1.06], [2.06, 1.556]], [[1, 1.495], [1.495, 1], [2.556, 2.06], [2.06, 2.556]]]

# [[5.551115123125783e-17, 0.6062177826491071], [0.3499999999999999, 0.0], [1.6490381056766577, 0.7499999999999997], [1.2990381056766578, 1.3562177826491069]]
